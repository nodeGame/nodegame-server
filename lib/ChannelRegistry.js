...
ChannelRegistry.prototype.addPlayer = function(playerId) {
	if (this.players.hasOwnProperty(playerId)) {
		node.warn('ChannelRegistry: Trying to add existent player');
		return false;
	}
 ...
 *
 * TODO: Update roomAliases, gameAliases
 */
ChannelRegistry.prototype.removePlayer = function(playerId) {
	if (!this.players.hasOwnProperty(playerId)) {
		node.warn('ChannelRegistry: Trying to remove non-existent player');
		return false;
	}
...
ChannelRegistry.prototype.movePlayer = function(player, newRoom) {
	...
	if ('undefined' === typeof newRoom) {
		// Try to pop player's room stack:
