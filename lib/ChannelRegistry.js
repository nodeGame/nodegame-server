...
const J          = ngc.JSUS;
const PlayerList = ngc.PlayerList;
const Player     = ngc.Player;
...
ChannelRegistry.prototype.generateClientId = function() {
    ...
    // base64 is more efficient, but it contains
    // characters that one should escape
    // return crypto.randomBytes(16).toString("base64");

    // Output like: baed5ac8bc802bd5348f4393
 ...
 * @param {string} client The client's alias or ID
 */
ChannelRegistry.prototype.checkOut =
...
ChannelRegistry.prototype.lookupClient = function(client, room) {
    ...
    if ('string' !== typeof client) {
        throw new Error('ChannelRegistry.lookupClient: client id must be ' +
                        'string. Found: ' + client);
    }
    if (client.trim() === '') {
        throw new Error('ChannelRegistry.lookupClient: empty client id');
    }
...
ChannelRegistry.prototype.authorize = function(profile) {
    // Username.
    let client = this.getClient(profile.id);
...
function generateClient(clientObj) {
    ...
    if ('undefined' === typeof client.win) client.win = 0;
    return client;
}
