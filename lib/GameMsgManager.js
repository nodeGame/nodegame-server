...
GameMsgManager.prototype.sendHI = function (text, to) {
	if (!to) {
		this.log.log('Attempt to send HI to nobody');
		return;
	}
 ...
 */
GameMsgManager.prototype.sendMLIST = function (node, to) {
	to = to || 'ALL';
	// TODO: set/get/say choose carefully
	var mlMsg = this.gmg.createMLIST(GameMsg.actions.SAY, node.pl, to);
	this.send(mlMsg);
};
