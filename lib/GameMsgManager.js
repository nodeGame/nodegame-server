...
function GameMsgManager(server) {
	...
	this.msg = this.gmg = this.generator = new GameMsgGenerator(this.session, server.name, new GameStage());	// TODO: Check what is best to init
...
// * @param {GameStage} state The state to send 
...
/**
 * ### GameMsgManager.sendSTAGE
 * 
 * Creates a STAGE message and sends it
 * 
 * @param {string} action The action type associated to the message (set/get/say)
 * @param {GameStage} stage The stage to send 
 * @param {string} to Optional. The recipient of the STAGE message. Defaults, 'ALL'
 */
GameMsgManager.prototype.sendSTAGE = function (action, stage, to) {
	to = to || 'ALL';
	// <!-- TODO: set/get/say choose carefully --!>
	var msg = this.msg.create({
		action: action,
...
/**
 * ### GameMsgManager.forwardSTAGE
 * 
 * Creates a STAGE message consistent with input parameters and forwards it
 * either to the specified recipient or to 'ALL'
 * 
 * @param {string} action The action type associated to the message (set/get/say)
 * @param {GameStage} stage The stage to send 
 * @param {string} to Optional. The recipient of the STAGE message. Defaults, 'ALL' 
 */
GameMsgManager.prototype.forwardSTAGE = function (action, stage, to) {
	to = to || 'ALL';
	var msg = this.msg.create({
		action: action,
		target: node.target.STAGE,
...
/**
 * ### GameMsgManager.forwardSTAGE_LEVEL
 * 
 * Creates a STAGE_LEVEL message consistent with input parameters and forwards it
 * either to the specified recipient or to 'ALL'
 * 
 * @param {string} action The action type associated to the message (set/get/say)
 * @param {GameStage} stage The stage to send 
 * @param {string} to Optional. The recipient of the STAGE message. Defaults, 'ALL' 
 */
GameMsgManager.prototype.forwardSTAGE_LEVEL = function (action, stage, to) {
    to = to || 'ALL';
    var msg = this.msg.create({
        action: action,
