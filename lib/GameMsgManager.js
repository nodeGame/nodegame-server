...
GameMsgManager.prototype.sendPLIST = function (server, to) {
	...
	var plMsg = this.gmg.createPLIST(GameMsg.actions.SAY, server.pl, to);
	this.send(plMsg);
};
...
GameMsgManager.prototype.sendMLIST = function (server, to) {
	to = to || 'ALL';
	// TODO: set/get/say choose carefully
	var mlMsg = this.gmg.createMLIST(GameMsg.actions.SAY, server.pl, to);
	this.send(mlMsg);
};
...
GameMsgManager.prototype.send = function (gameMsg) {
	...
	if (to === 'ALL') {
		if (rel) {
			this.server.channel.json.send(msg);
		} 
		else {
//			var v =  this.server.channel;
//			for (var i in v) {
//				if (v.hasOwnProperty(i)){
//					log(v[i]);
//				}
//			}
			this.server.channel.volatile.json.send(msg);
			//this.server.channel.broadcast.volatile.json.send(msg);
		}
...
GameMsgManager.prototype.broadcast = function (gameMsg) {
	...
	for (var client in this.server.channel.sockets) {
		if (this.server.channel.sockets.hasOwnProperty(client)) { 
			if (client !== from) {
			...
			// <!-- 
			//log(this.server.channel.sockets[client]);
			//this.server.channel.sockets[client].send(msg);
			// --!>
		}
	}
};
...
GameMsgManager.prototype.forward = function (gameMsg) {
	...
	this.server.partner.gmm.send(gameMsg);
