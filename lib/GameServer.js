...
GameServer.prototype.onConnect = function(socketId, socketObj, headers,
    ...
    // TODO: remove
    // if (socketId.charAt(0) !== 'D') debugger

    var decoded, res, clientId;
    ...
    if (cookies && cookies.nodegame_token) {
        ...
        if (decoded) {
            cookies.session = decoded.session;
            cookies.player = decoded.player;
    ...
    if (clientId) {
        ...
        // at least once to be a reconnection.
        if (this.enableReconnections &&
            this.registry.isUsed(clientId)) {

            res = this.handleReconnectingClient(clientId, socketId,
        ...
        // Generate a new unique client id (assume this always works).
    ...
    if (newConnection) {
        ...
        res = this.handleConnectingClient(clientId, socketId, socketObj,
                                          headers, cookies,
                                          invalidSessionCookie,
                                          clientType, startingRoom);
    }
 ...
 *
 * @param {string} socketId The client id of the socket connection
 * @param {string} socketId The id of the socket connection
 * @param {Socket} socketObj The socket object (Direct or SIO)
 * @param {object} headers The connection headers passed by the Socket
...
GameServer.prototype.handleConnectingClient = function(
    clientId, socketId, socketObj, headers, cookies,
    invalidSessionCookie, clientType, startingRoom) {

    var sys;
    var matchedId, clientObj, decorClientObj, res;

    matchedId = null;
    sys = this.sysLogger;

    // Try to match client ID with client in registry, or create a new one.
    clientObj = this.registry.getClient(clientId);
    if (clientObj) matchedId = true;
    else clientObj = { id: clientId };
    ...
    // Adds or updates the client in registry.
    if (matchedId) {
        this.registry.updateClient(clientId, clientObj);
    }
    else {
    ...
    if (!this.channel.placeClient(clientObj, startingRoom)) {
    ...
    sys.log('channel ' + this.channel.name + ' ' +
            (this.ADMIN_SERVER ? 'admin' : 'player') + ' connected: ' +
            clientId);
...
GameServer.prototype.handleReconnectingClient = function(
    ...
    client = this.registry.getClient(clientId);
    ...
    if (!this.channel.placeClient(clientObj, returningRoom)) {
        return GameServer.codes.ROOM_NOT_FOUND;
    }
