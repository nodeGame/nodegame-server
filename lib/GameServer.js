 ...
 *
 * _to_ is verified in GameServer.validateRecipient.
 *
 ...
 * field.
 *
 * @param {GameMsg} The message to validate
 *
...
GameServer.prototype.validateRecipient = function(gameMsg) {
    ...
    if ('string' === typeof gameMsg.to) {        
        ...
        if (gameMsg.to === 'ALL' && !this.canSendTo.all) {
            errStr += 'ALL/';
            gameMsg.to = 'CHANNEL';
        }

        if (gameMsg.to === 'CHANNEL' && !this.canSendTo.ownChannel) {
            errStr += 'CHANNEL/';
            gameMsg.to = 'ROOM';
        }

        if (gameMsg.to === 'ROOM' && !this.canSendTo.ownRoom) {
            this.sysLogger.log(server + 'Server.validateRecipient: ' +
                               'sending to "' + errStr +
        ...
        if ((gameMsg.lastIndexOf('CHANNEL_', 0) === 0) && !this.canSendTo.anyChannel
            this.sysLogger.log(server + 'Server.validateRecipient: ' +
                               'msg.to begins with "CHANNEL_" illegally.',
                               'error');
            return false;
        }

        if ((gameMsg.lastIndexOf('ROOM_', 0) === 0) && !this.canSendTo.anyRoom) {
            this.sysLogger.log(server + 'Server.validateRecipient: ' +
                               'msg.to begins with "ROOM_" illegally.',
                               'error');
            return false;
        }
        ...
        if (gameMsg.to.length > this.maxNumRecipients) {
            this.sysLogger.log(server + 'Server.validateRecipient: ' +
                               'number of recipients exceeds maxNumRecipients ' +
                               'limit. ' + gameMsg.to.length + '>' + 
                               this.maxNumRecipients, 'error');
            return false;    
        }
