...
//	});
	...
	this.channel = this.sio.of(this.endpoint).on('connection',
		function(socket) {
			// Register the socket as a class variable
			that.socket = socket;
			that.gmm.socket_manager.register(socket);
...
//					that.log.log('JUST RECEIVED P ' + util.inspect(msg));
					...
					that.sysLogger.log(msg.toEvent() + ' ' + msg.from + '-> ' + msg.to);
	...
	this.sio.sockets.on("shutdown", function(message) {
		that.sysLogger.log("Server is shutting down.");
		that.pl.clear(true);
