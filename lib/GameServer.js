...
debugger
    ...
    if (this.enableReconnections && cookies.session === this.channel.session) {

        // Client must have connected at least once to be a reconnection.
        if (this.registry.isUsed(cookies.player)) {
...
GameServer.prototype.handleConnectingClient = function(
    ...
    if (this.generateClientId) {
        ...
        if (this.registry.clients.exist(clientId) &&
            !this.enableReconnections) {

            sys.log('GameServer.handleConnectingClient: generateClientId ' +
                    'returned an existing ID for incoming ' +
                    (this.ADMIN_SERVER ? 'admin' : 'player' ) +
                    ', but reconnections are disabled.', 'error');

            return GameServer.codes.DUPLICATED_CLIENT_ID;
        }

        if (this.registry.isUsed(clientId)) {

            res = this.handleReconnectingClient(clientId, socketId, socketObj,
                                                clientType, cookies, headers);
    ...
    if (!this.registry.getClient(clientId)) {
