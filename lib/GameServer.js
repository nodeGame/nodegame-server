...
GameServer.prototype.onDisconnect = function(sid, socket) {
    ...
    console.log(this.registry.clients.db);
    console.log(sid);
 ...
 * @param {object} handshake An object containing information about the
 *   connection, including headers, query fields and startingRoom
...
GameServer.prototype.onConnect = function(
    socketId, socketObj, headers, startingRoom, clientType) {
    ...
    if (this.authCb) {
        if (!this.authCb(headers, cookies, startingRoom, clientType)) {
 ...
 * @param {string} startingRoom Optional. The name of the room in which
 *    the client is to be placed. Default: Waiting room
 * @param {string} clientType Optional. The type of the client
 *   (player, bot, etc.). Default: 'default'
 *
 * @return {number} A numeric code describing the outcome of the operation.
 *
 * @see GameServer.codes
...
GameServer.prototype.handleConnectingClient = function(
    ...
    invalidSessionCookie, startingRoom, clientType) {
    ...
    console.log('### sid:', socketId, ', type:', clientObj.type); // DEBUG
