...
GameServer.prototype.onConnect = function(
    ...
    if (newConnection) {
        ...
        if ('undefined' === typeof clientId) {
            sys.log('GameServer.onConnect: registry.addClient did not return ' + 
                    'an id for incoming ' + 
                    (this.ADMIN_SERVER ? 'admin' : 'player' ), 'error');

            // Since the client is not yet registered, we must use the socket
            // object passed along in the connection to send him a msg with
            // his socket id.
            socketObj.send(this.msg.create({
                target: ngc.constants.target.ALERT,
                to: 'unregistered_client',
                text: 'An error has occurred on the server while trying to ' +
                    'register the connection. Please try again later or ' +
                    'an administrator.'
            }), socketId);
            return false;
        }
