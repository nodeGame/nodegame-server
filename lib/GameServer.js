...
function GameServer(options) {
	...
	//this.log = new ServerLog(this.user_options);
	
	//this.log = this.server.loggers.get('channel');
	
	this.sysLogger = winston.loggers.get('channel');
	this.msgLogger = winston.loggers.get('messages')
...
GameServer.prototype.attachListeners = function() {
	...
	this.log.log('Listening for connections');
...
//	});
		...
		function(socket) {
			...
			socket.on('message', function(message) {
				...
				if (msg) { // Parsing Successful
					...
					that.log.log(that.name + ' About to emit ' + msg.toEvent());
					that.log.log(msg.toEvent() + ' ' + msg.from + '-> ' + msg.to);
	...
	this.server.sockets.on("shutdown", function(message) {
		that.log.log("Server is shutting down.");
		that.pl.clear(true);
