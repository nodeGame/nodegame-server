...
GameServer.prototype.onDisconnect = function(sid, socket) {
    ...
    res = this.registry.markDisconnected(client.id);
    
    if (!res) {
        clientName = client.id + ' (sid: ' + sid + ')';
        throw new Error('GameServer.onDisconnect: an error has occurred ' + 
                        'while marking client ' + clientName + ' disconnected.');
    }
