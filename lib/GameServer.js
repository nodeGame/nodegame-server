...
const GameMsgGenerator = require('./GameMsgGenerator');
const SocketIo = require('./sockets/SocketIo');
const SocketDirect = require('./sockets/SocketDirect');
...
GameServer.prototype.onDisconnect = function(sid, socket) {
    ...
    // TODO: is this what we should do? Isn't enough to mark disconnected?
    room.clients.remove(client.id);

    // MODS.
    if (!room.haveDisconnected) room.haveDisconnected = {};
    room.haveDisconnected[client.id] = client;
    // END MODS.
...
GameServer.prototype.onConnect = function(socketId, socketObj, handshake,
    ...
    // Try to decode JWT only if auth or no-auth-cookie is on.
    let info = this.channel.gameInfo;
    ...
    if (socketObj.name !== 'direct') {
        ...
        if (res && res.enabled && res.mode !== 'monitor' &&
           (!clientId || invalidSessionCookie)) {
    ...
    if (this.channel.options.collectIp) {
        let h = handshake.headers || {}
        if (!h['x-real-ip']) h['x-real-ip'] = null;
        if (!h['x-forwarded-for']) h['x-forwarded-for'] = null;
...
GameServer.prototype.registerClient = function(clientObj, socketObj, room,
    ...
    // Connected time.
    clientObj.connectTime = new Date();
