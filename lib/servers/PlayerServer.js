...
// Listens on newly connected players
    this.on('connecting', function(clientId, socketId) {
        ...
        if (!room) {
            sys.log('Could not determine room for incoming player: ' + clientId);
            return;
        }
    ...
    this.on(say + 'HI', function(msg) {
        ...
        if (!room) {
            sys.log('Could not determine room for incoming player: ' + msg.from);
            return;
        }
...
// TODO
    this.on(say + 'PLAYER_UPDATE', function(msg) {
        ...
        // <!-- Do we need this? --!>
        //that.pl.updatePlayer(msg.from, msg.data);
