    ...
    this.on(say + 'TXT', function(msg) {
        ...
        if (msg.to === 'ALL') {  // maybe 'ROOM'?
            ...
            if (msg.to === 'SERVER' || that.forwardAll) {
                ...
                if (!room) {
                    sys.log('PlayerServer.on.say.TXT: Could not determine room of msg sender: ' + msg.from, 'error');
                    return;
                }
    ...
    this.on(say + 'DATA', function(msg) {
        ...
        if (msg.to === 'ALL') {  // maybe 'ROOM'?
            ...
            if (msg.to === 'SERVER' || that.forwardAll) {
                ...
                if (!room) {
                    sys.log('PlayerServer.on.say.DATA: Could not determine room of msg sender: ' + msg.from, 'error');
                    return;
                }
...
// Listens on set.DATA messages
    this.on(set + 'DATA', function(msg) {
        ...
        if (!room) {
            sys.log('PlayerServer.on.set.DATA: Could not determine room of sender: ' + msg.from, 'error');
            return;
        }
...
// TODO
    this.on(say + 'PLAYER_UPDATE', function(msg) {
        ...
        if ((that.notify.onStageUpdate && msg.data.hasOwnProperty('stage')) ||
            ...
            if (!room) {
                sys.log('PlayerServer.on.say.PLAYER_UPDATE: Could not determine room of sender: ' + msg.from, 'error');
                return;
            }
...
// Listens on say.STAGE messages
    this.on(say + 'STAGE', function(msg) {
        ...
        if (that.notify.onStageUpdate) {
            ...
            if (!room) {
                sys.log('PlayerServer.on.say.STAGE: Could not determine room of sender: ' + msg.from, 'error');
                return;
            }
...
// Listens on say.STAGE_LEVEL messages
    this.on(say + 'STAGE_LEVEL', function(msg) {
        ...
        if (that.notify.onStageUpdate) {
            ...
            if (!room) {
                sys.log('PlayerServer.on.say.STAGE_LEVEL: Could not determine room of sender: ' + msg.from, 'error');
                return;
            }
...
// Listens on players disconnecting
    this.on('disconnect', function (player) {
        ...
        if (that.notify.onConnect) {
            ...
            if (!room) {
                sys.log('PlayerServer.on.disconnect: Could not determine room for disconnecting player: ' + msg.from, 'error');
                return;
            }
