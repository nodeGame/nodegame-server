...
PlayerServer.prototype.attachCustomListeners = function() {
    ...
    this.on('connecting', function(clientObj, roomName) {
        var room, players, admins, oldPlayers, clientId, socketId;
        var filteredPlayers;
        var pConnectMsg;
        ...
        // setup allows it.
        if (room.acm.notify.onConnect && oldPlayers.db.length > 0) {
            // Notify all the connected players that a new one just connected.
            pConnectMsg.data = {
                id:  clientId
            };
            that.socketManager.broadcast2group(pConnectMsg, oldPlayers);
   ...
   // #### say.DATA
    ...
    this.on('disconnect', function(player, room) {
        ...
        if (room.acm.notify.onConnect) {
            that.socketManager.send2room(msg, room);
        }
        else {
            that.socketManager.send2roomAdmins(msg, room);
        }
