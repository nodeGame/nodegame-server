...
AdminServer.prototype.attachCustomListeners = function() {
    ...
    this.on('SERVERCOMMAND_INFO', function(data, from) {
        ...
        if (data.type === 'CHANNELS') {
            ...
            replyData = { clients: roomObj.clients.db };

            logicObj = roomObj.node;
            // Some rooms, e.g. GarageRoom, have no logic.
            if (logicObj) {
                logicObj = that.registry.getClient(logicObj.player.id);

                // TODO: paused is not in registry. Should it?

                replyData.logic = {
                    stage: logicObj.stage,
                    stageLevel: logicObj.stageLevel,
                    stateLevel: logicObj.stateLevel,
                    id: logicObj.id,
                    sid: logicObj.sid,
                    admin: true,
