...
// Listens on say.PLIST messages	
	...
	// TODO Ugly change API
	var plMsg = that.msg.gmg.sayPLIST(that.partner.pl, msg.to);
	that.msg.forward(plMsg);
	that.msg.socket_manager.broadcast(plMsg, msg.from);
    });    
...
// ### say.STAGE_LEVEL
// Listens on say.STATE_LEVEL messages 
    this.on(say + 'STAGE_LEVEL', function(msg) {
        // Send it to players and other monitors
        that.msg.forwardSTAGE_LEVEL(action.SAY, msg.data, msg.to);
        that.msg.broadcast(msg, msg.from);
    });
...
// Forwards a redirect msg (only for admin)
	this.on(say + 'REDIRECT', function (msg) {
		// TODO refactor msg
		msg = that.msg.gmg.sayREDIRECT(msg.data, msg.to);
