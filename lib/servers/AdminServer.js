...
AdminServer.prototype.attachCustomListeners = function() {
    ...
    this.on(say + 'DATA', function(msg) {
        ...
        if (!msg.to) {
            sys.log('AdminServer.on.say.DATA: Invalid "to" field "' +
                msg.to + '"', 'error');
            console.log('TO FIELD ERROR');
            return;
        }

        if (msg.to === 'ALL') {  // maybe 'ROOM'?
            that.socket.broadcast(that.msg.obfuscate(msg));
        }
        else {
            if (msg.to === 'SERVER') {
    ...
    this.on(say + 'REDIRECT', function(msg) {
        ...
        if (!room) {
            sys.log('AdminServer.on.say.REDIRECT: Could not determine room ' +
                    'of msg sender: ' + msg.from, 'error');
            return;
        }
    ...
    this.on(say + 'SETUP', function(msg) {
        ...
        if (!room) {
            sys.log('AdminServer.on.say.SETUP: Could not determine room of ' +
                    'msg sender: ' + msg.from, 'error');
            return;
        }
    ...
    this.on('disconnect', function (monitor) {
        ...
        if (!room) {
            sys.log('AdminServer.on.disconnect: Could not determine room for ' +
                    'disconnecting monitor: ' + msg.from, 'error');
            return;
        }
