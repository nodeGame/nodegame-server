...
AdminServer.prototype.attachCustomListeners = function() {
    ...
    this.on('SERVERCOMMAND_ROOMCOMMAND', function(data, from) {
        ...
        // Check doPlayers field
        if (('undefined' !== typeof data.doPlayers) &&
            ('boolean' !== typeof data.doPlayers)) {

            sys.log('AdminServer.on.SERVERCOMMAND_ROOMCOMMAND: data has ' +
                    'invalid doPlayers field.', 'error');
            return;
        }
        ...
        if (data.type === 'SETUP') {
            ...
            roomObj.startGame(data.doPlayers);
        }
        else if (data.type === 'PAUSE') {
            roomObj.pauseGame(data.doPlayers);
        }
        else if (data.type === 'RESUME') {
            roomObj.resumeGame(data.doPlayers);
        }
        else if (data.type === 'STOP') {
            roomObj.stopGame(data.doPlayers);
        }
        else {
            sys.log('AdminServer.on.SERVERCOMMAND_ROOMCOMMAND: data has ' +
    ...
    this.on('SERVERCOMMAND_INFO', function(data, from) {
        ...
        if (data.type === 'CHANNELS') {
            ...
            channelObj = that.servernode.channels[data.channel];
            for (room in channelObj.gameRooms) {
                if (channelObj.gameRooms.hasOwnProperty(room)) {
                    roomObj = channelObj.gameRooms[room];
            ...
            // TODO: Send only specific data, not everything; send _paused_ field
            replyData = roomObj.clients.db;
        ...
        if (msg.to === 'SERVER') {
            ...
            if (!room) {
                sys.log('AdminServer.on.say.DATA: Could not determine ' +
                        'room of msg sender: ' + originalFrom, 'error');
                return;
            }
            admins = room.clients.admin;
...
AdminServer.prototype.standardAdmin2PlayerMsg = function(msg) {
    ...
    if (msg.to === 'SERVER') {
        sys.log('AdminServer.on.' + msg.toEvent() + ': "SERVER" is not a ' +
                'valid recipient.', 'error');
        return;
    }
    ...
    if (msg.to === 'ALL') {
        ...
        if (!room) {
            sys.log('AdminServer.on.' + msg.toEvent() + ': Could not ' +
                    'determine room of msg sender: ' + originalFrom, 'error');
            return;
        }
