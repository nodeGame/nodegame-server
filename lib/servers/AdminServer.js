...
AdminServer.prototype.attachCustomListeners = function() {
   ...
   this.on('SERVERCOMMAND_EXPORT', function(data, from) {
       ...
       const nggDir = J.resolveModuleDir('nodegame-generator');
       const nodegame = path.resolve(nggDir, 'bin', 'nodegame');

       let cmd = [ nodegame ];
       if (data.type === 'DATA') {
           cmd = [ ...cmd, 'export-data', that.channel.name ];
       }
       else {
           cmd.push('export-logs');
       }
       ...
       let child = execFile(process.execPath, cmd,
           // { cwd: ROOT_DIR },
           (error, stdout, stderr) => {
