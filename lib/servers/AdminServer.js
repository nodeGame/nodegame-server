...
AdminServer.prototype.attachCustomListeners = function() {
    ...
    this.on(say + 'DATA', function(msg) {
        ...
        if (msg.to === 'ALL') {  
            that.socket.broadcast2all(msg, originalFrom);
        }
        else if (msg.to === 'CHANNEL') {
            that.socket.broadcast2ownChannel(msg, originalFrom);
        }
        else if (msg.to === 'ROOM') {
            that.socket.broadcast2ownRoom(msg, originalFrom);
        }
        else if (msg.to === 'CHANNEL_X') {
            that.socket.broadcast2channel(msg, msg._to, originalFrom);
        }
        else if (msg.to === 'ROOM_X') {
            that.socket.broadcast2room(msg, msg._to, originalFrom);
        }
        else {
            if (msg.to === 'SERVER') {
                // Send to admins in the room:
                admins = room.clients.admin;
                that.socket.broadcast2group(msg, admins, originalFrom);
            }

            if (msg.to !== 'SERVER') {
                // Send to receiving player:
                // TODO: isn't obfuscated before?
                that.socket.send2client(that.msg.obfuscate(msg));
            }
        }
...
// TODO: continue refactor here. 30.03.2014
    ...
    this.on('SERVERCOMMAND_INFO', function(data, from) {
        ...
        if (data.type === 'CHANNELS') {
            ...
            // Get room object:
            if (!channelObj.gameRooms.hasOwnProperty(data.room)) {
                sys.log('AdminServer.on.SERVERCOMMAND_INFO.CLIENTS: data has ' +
                        'invalid room field');
                return;
            }
            roomObj = channelObj.gameRooms[data.room];

            // Fill replyData with client info:
            // TODO: Send only specific data, not everything
            replyData = J.clone(roomObj.clients.db);
