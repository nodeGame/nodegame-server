...
// Listens on say.STATE messages 
	this.on(say + 'STATE', function(msg) {
		...
		that.msg.forwardSTATE (GameMsg.actions.SAY,msg.data, msg.to);
		that.msg.broadcast(msg, msg.from);
		//that.msg.sendSTATE (GameMsg.actions.SAY,msg.data, msg.to);
...
// Listener on get (Experimental)	
	this.on(get + 'DATA', function (msg) {
		...
		if (msg.text === 'LOOP') {
			that.msg.sendDATA(GameMsg.actions.SAY, that.loop, msg.from, 'LOOP');
		}		
		
		else if (msg.text === 'INFO') {
			that.msg.sendDATA(GameMsg.actions.SAY, that.generateInfo(), msg.from, 'INFO');
		}
