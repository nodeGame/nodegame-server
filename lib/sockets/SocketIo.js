...
//            console.log('hello!', socket.handshake.decoded_token.name);
            ...
            let opts = that.gameServer.options;
            let prefixedSid = that.sidPrefix + socket.id;

            let startingRoom, clientType;
            if (opts.sioQuery && socket.handshake.query) {
            ...
            let res = that.gameServer.onConnect(prefixedSid, that,
...
SocketIo.prototype.disconnect = function(sid) {
    ...
    socket = this.sioChannel.sockets.get(sid.slice(2));
...
SocketIo.prototype.send = function(gameMsg, sid, broadcast) {
    var msg, client, rel, strippedSid;
    ...
    // Either send to a specific client(1), or to ALL but a specific client(2).
        ...
        if (!broadcast) {
            ...
            client = this.sioChannel.sockets.get(strippedSid);
        ...
        // (2)
            ...
            // for (clientSid in this.sioChannel.sockets) {
            this.sioChannel.sockets.forEach((client, clientSid) => {

                if (strippedSid === clientSid) return;
