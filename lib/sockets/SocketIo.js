...
SocketIo.prototype.send = function(gameMsg, sid) {
    ...
    if (to === 'SERVER' || to === null) {
        this.game_server.sysLogger.log('SocketIo.send: Trying to send msg to ' +
                                       'nobody: ' + to, 'error');
        return false;
    }
    ...
    if (to === 'ALL') {
        ...
        this.game_server.sysLogger.log('Msg ' + gameMsg.toSMS() +
                                       ' sent to ' + to);
        this.game_server.msgLogger.log(gameMsg);
    }
    // Send to a specific client.
        ...
        if (client) {
            ...
            this.game_server.sysLogger.log('Msg ' + gameMsg.toSMS() + 
                                           ' sent to sid' + strippedSid);
            this.game_server.msgLogger.log(gameMsg);
        }
        else {
            this.game_server.sysLogger.log('SocketIo.send: Msg not sent. ' +
                                           'Unexisting recipient sid: ' + 
                                           strippedSid, 'error');
            return false;
        }
    }
