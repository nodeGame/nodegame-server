...
function SocketIo(gameServer) {
     ...
     *
     * Sids are represented as SA/channelName#sid, but in sockets.connected
     * they are saved as /#sid
     *
     ...
     */
    this.sidPrefixStrip = this.sidPrefix + '/' + this.gameServer.channel.name;
...
SocketIo.prototype.disconnect = function(sid) {
    ...
    strippedSid = '/' + sid.slice(this.sidPrefixStripLen);
...
SocketIo.prototype.send = function(gameMsg, sid, broadcast) {
    ...
    // Either send to a specific client(1), or to ALL but a specific client(2).
        ...
        if (!broadcast) {
            ...
            // Ste: Was:
            // client = this.sioChannel.sockets[strippedSid];
