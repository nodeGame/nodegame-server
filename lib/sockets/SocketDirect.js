...
SocketDirect.prototype.connect = function(clientSocket, options) {
    ...
    res = this.game_server.onConnect(sid, this, options.headers, options.startingRoom);
...
SocketDirect.prototype.send = function(msg, sid) {
    ...
    if (to === 'SERVER' || to === null) {
        this.game_server.sysLogger.log('SocketDirect.send: Trying to send msg to nobody: ' + to, 'error');
        return false;
    }
    ...
    try {
        ...
        this.game_server.sysLogger.log('SocketDirect.send: An error has occurred. Cannot send message: ' + msg, 'error');
        return false;
    }
    ...
    if (to === 'ALL') {
        ...
        this.game_server.msgLogger.log(msg);
    }
    // Send to a specific client
        ...
        if (client) {
            ...
            this.game_server.sysLogger.log('SocketDirect.send: msg not sent.' +
                                           'Unexisting recipient: ' + to, 'error');
            return false;
        }
    }
    return true;
