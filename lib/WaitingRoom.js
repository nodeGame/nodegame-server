...
WaitingRoom.prototype.parseSettings = function(settings) {
    ...
    if (this.POOL_SIZE < this.GROUP_SIZE) {
        throw new TypeError(where + 
                'POOL_SIZE should be larger or equal to GROUP_SIZE');
    }
...
/**
 * ### WaitingRoom.isDispatchable
 *
 * Checks whether another game can be dispatched
 */

WaitingRoom.prototype.isDispatchable = function() {
    if (this.N_GAMES) {
...
WaitingRoom.prototype.dispatch = function() {
    ...
    return function(msg, numberOfGames) {
        ...
        for (gameNumber = 0; gameNumber < numberOfGames; ++i) {
            if (nPlayers < this.GROUP_SIZE) {
                throw new Error(where + 'Attempting to dispatch game for ' +
                    this.GROUP_SIZE + ' players with only ' + nPlayers + 
                    ' participants.');
            }
            ...
            if (!this.isDispatchable()) {
                this.channel.sysLogger.log(where +
                       'Number of dispatches already performed: ' +
                       this.numberOfDispatches +
                       ' Number of specified games: ' + this.N_GAMES +
            ...
            for (i = 0; i < nPlayers; i++) {
                ...
                // Players that may not join.
                    ...
                    code = that.channel.registry.getClient(shuffledPList.db[i].id);
                    this.node.say('DISPATCH', shuffledPList.db[i].id, {
                        action: "NotSelected",
                        exit: code.ExitCode, 
