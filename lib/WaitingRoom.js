 ...
 */
WaitingRoom.prototype.kickUnresponsivePlayers = function(callable) {
    return function(callback, options) { 
        ...
        if (!callable) {
            console.log('Warning: Ignoring call to ' + where + 
            ...
            that.node.get('PING',
                ...
                p.id, 
                {
                    ...
                    timeoutCb: function(p) {
                        return function() {
                            console.log('Player ' + p.id + 'unresponsive. Kicking.');
...
WaitingRoom.prototype.dispatch = function() {
    ...
    return function(msg, numberOfGames) {
        ...
        that.kickUnresponsivePlayers(function () {
            ...
            for (gameNumber = 0; gameNumber < numberOfGames; ++gameNumber) {
                ...
                for (i = 0; i < nPlayers; i++) {
                    ...
                    // Players that may not join.
                        ...
                        that.node.say('DISPATCH', shuffledPList.db[i].id, {
                            action: "NotSelected",
                            exit: code.ExitCode, 
                            isDispatchable: that.isDispatchable()
                        });
