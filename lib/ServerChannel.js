...
ServerChannel.prototype.createGameRoom = function(conf) {
    ...
    if ('undefined' !== typeof conf.channel) {
        this.logger.syslog("ServerChannel.createGameRoom: channel parameter ignored", 'warn');
    }
    ...
    if (playerList) {
        playerIds = playerList.id.getAllKeys();
        for (i in playerIds) {
...
ServerChannel.prototype.createWaitingRoom = function(conf) {
    ...
    if ('undefined' !== typeof conf.channel) {
        this.logger.syslog("ServerChannel.createWaitingRoom: parameter 'channel' ignored", 'warn');
    }

    if ('undefined' !== typeof conf.name) {
        this.logger.syslog("ServerChannel.createWaitingRoom: parameter 'name' ignored", 'warn');
    }
...
ServerChannel.prototype.placePlayer = function(playerObj, inRoom) {
    var pConnectMsg, playerId, fromRoom;
    ...
    // Add player to new room:
    this.gameRooms[inRoom].playerList.add(playerObj);

    // Update ChannelRegistry:
    this.registry.moveClient(playerId, inRoom);

    // TODO: Send updated PLISTs to all involved players
    return
    ...
    if (that.notify.onConnect) {
        ...
        that.socket.send(that.msg.create({
            ...
            to:     msg.from,
            text:   'plist',
            data:   J.stringify([that.getPlayerList(), 'append'])
        }));
    }
