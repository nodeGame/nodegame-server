 ...
 * TODO
 */
ServerChannel.prototype.connectBot = function(room, botFile, mixinConf) {
    var bot;
    var botPath;
    var settings;
    ...
    //  console.log('*** ', node.game.getStateLevel(), node.game.getStageLevel(), node.game.getCurrentGameStage());
    //}, 500);
    this.bots.push(node);
    console.log("PUSHED", this.bots[0].nodename);

    //watch(node.socket, 'connected', function(){console.log('*** CONNECTED CHANGED to', node.socket.connected);});
    watch(node, 'nodename', function(){console.log('*** NAME -->', node.nodename);});

    botPath = this.servernode.resolveGameDir(gameName) + botFile;

    game = this.servernode.getGamesInfo(gameName);
    settings = game.treatments[treatmentName];

    // Require bot.
    bot = require(botPath)(node, room, treatmentName, settings);
    this.bots.push(bot);
    ...
    // TESTING
    node.player.type = 'myType';

    // Connect bot to server.
    node.socket.setSocketType('SocketDirect', {
        socket: this.player.socket.sockets.direct
    });
    node.connect(null, null, { startingRoom: room.name, type: 'myType' });
    console.log('BOT CONNECTED');

    //node.remoteSetup('nodegame', node.player.id, bot);
};
