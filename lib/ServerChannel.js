 ...
 *    the players to.
 *    If null, then the room will not be deleted if it has players.
 *    Default: null
 *
 ...
 *   - phantomjs (boolean)
 *      Optional. Whether to emulate a browser connection with a PhantomJS
 *      process instead of a direct server connection. The remaining fields are
 *      only used if this is FALSE. Default: FALSE.
 *   - clientType (string)
...
ServerChannel.prototype.connectBot = function(options, mixinConf) {
    ...
    if ('undefined' !== typeof options.phantomjs &&
        'boolean'   !== typeof options.phantomjs) {
        throw new TypeError('ServerChannel.connectBot: ' +
                'options.phantomjs must be boolean or undefined.');
    }
    phantomjs = 'undefined' === typeof options.phantomjs ?
        false : options.phantomjs;
    ...
    if (phantomjs) {
        ...
        serverdir = that.servernode.rootDir;
        url = 'http://localhost:' + that.servernode.port + '/' + that.name +
            '/?auto=true';
        logger = Logger.get('clients', {name: 'phantomjs'});
        phantomjs = spawn(serverdir + '/node_modules/phantomjs/bin/phantomjs',
            [serverdir + '/phantomjs/openurl.js', url]);
