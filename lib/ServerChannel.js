...
ServerChannel.prototype.connectBot = function(options) {
    ...
    // TODO: now we use options.setup twice: in here, and we pass it
    // here, and later room.setupClient(bot.player.id, options.setup);
    // There is no harm, but we could improve this. We need to use
    // options.setup.settings to mixin and pass it the client type cb.
    // Maybe we need to separate them.
    //
    ...
    bot.once('PLAYER_CREATED', function() {
        ...
        if (init) {
            ...
            room.setupClient(bot.player.id, options.setup);
