...
ServerChannel.prototype.movePlayer = function(playerId, toRoom) {
    ...
    pConnectMsg = this.playerServer.msg.create({
        target: 'PCONNECT'
    });

    pDisconnectMsg = this.playerServer.msg.create({
        target: 'PDISCONNECT'
    });

    if (this.playerServer.notify.onConnect) {
        // Notify all the  players that a new one just connected
        pConnectMsg.data = {id: playerObj.id};
        this.playerServer.socketManager.broadcast2room(pConnectMsg,
                                                       toRoomObj, playerId);

        // Send the list of connected players to the new player
        filteredPlayers = oldPlayerList.fetchSubObj('id');
        this.playerServer.socketManager.send(this.playerServer.msg.create({
            target: 'SETUP',
            to:     playerId,
            text:   'plist',
            reliable:500,
        ...
        this.playerServer.socketManager.broadcast2room(pDisconnectMsg,
                                                       this.gameRooms[fromRoom],
                                                       playerId);
    }
    else {
        pConnectMsg.data = playerObj;
        this.playerServer.socketManager.send2roomAdmins(pConnectMsg, toRoomObj);
        pDisconnectMsg.data = playerObj;
        this.playerServer.socketManager
            .send2roomAdmins(pDisconnectMsg, toRoomObj);
    }
 ...
 *
 * @return {GameRoom|WaitingRoom} room The created room
 */
function createRoom(type, settings) {
    ...
    conf.name = settings.name || type + 'Room';
