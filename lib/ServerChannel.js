...
ServerChannel.prototype.destroyGameRoom = function(roomName, options) {
    ...
    playerIds = roomObj.clients.id.getAllKeys();
    ...
    // Check whether game in room is still running:
    if (!ignoreRunningGame &&
            roomObj.node.game.getStateLevel() <
            ngc.variables.stateLevels.GAMEOVER) {
                return false;
    }

    // Move players into substitute room:
    for (i in playerIds) {
...
ServerChannel.prototype.movePlayer = function(playerId, toRoom) {
    ...
    this.gameRooms[toRoom].clients.add(playerObj);
...
ServerChannel.prototype.placePlayer = function(playerObj, inRoom) {
    ...
    // Add player to room:
    this.gameRooms[inRoom].clients.add(playerObj);
