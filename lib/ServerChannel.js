...
ServerChannel.prototype.connectBot = function(options, mixinConf) {
    ...
    loadGame = 'undefined' === typeof options.loadGame ? true : options.loadGame;

    node = ngc.getClient();

    if (loadGame) {
        gameName = options.room.gameName;
        if (!gameName) {
            throw new TypeError('ServerChannel.connectBot: ' +
                                'options.room.gameName must exist if ' +
                                'options.loadGame is true.');
        }
        treatmentName = options.room.treatmentName;
        if (!treatmentName) {
            throw new TypeError('ServerChannel.connectBot: ' +
                                'options.room.treatmentName must exist if ' +
                                'options.loadGame is true.');
        }
