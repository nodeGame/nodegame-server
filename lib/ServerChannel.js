...
const AdminServer = require('./servers/AdminServer');
const PlayerServer = require('./servers/PlayerServer');
...
const GarageRoom = require('./rooms/GarageRoom');
const WaitingRoom = require('./rooms/WaitingRoom');
const RequirementsRoom = require('./rooms/RequirementsRoom');
...
ServerChannel.parseOptions = function(options) {
    ...
    out = J.clone(options);
    J.mixout(adminOptions, out);
    tmp = J.clone(options);
    J.mixout(playerOptions, tmp);
...
function ServerChannel(options, servernode, sio) {
     ...
     *
     * The registry of all connected and disconnected clients
     *
     * @see ChannelRegistry
     */
     ...
     * @see ServerChannel.logClients
     */
    this.logClientsInterval = options.logClientsInterval || 10000;
    ...
    // Experimental Code.
    // The purpose of this is to auto-fill frame properties in the stager,
    // when the frame is missing. It needs to be here, rather than inside
    // gameRoom to avoid replication. However, it is a bit of a duplicate
    // with the cache used by ResourceManager. The problem is that
    // ResourceManager starts filling the cache after first successful
    // request, instead we need to know the exact path beforehand. E.g.,
    // file.html vs file.htm vs file (for views). Alternatively,
    // ResourceManager could test both .html and .htm, so the auto-framing
    // would be dealt by the client instead of GameRoom.
    this.frameCache = {};
}
 ...
 *
 * Moves a client into a different room
 *
 ...
 *
 * Places a client into a room for the first time
 *
 * Client must not be found in any other room, otherwise
 * an error is raised.
 *
 ...
 *
 * @param {string} t The treatment to check
 *
 * @see ServerChannel.gameInfo
 *
 * TODO: This method does not look into levels
 */
...
ServerChannel.prototype.connectBot = function(options) {
    ...
    if (options.room) {
        if ('string' === typeof options.room) {
            ...
            if (!room) {
                throw new Error('ServerChannel.connectBot: options.room does ' +
                                ...
                                options.room + formatChanName(this));
            }
        }
            ...
            if (!this.gameRooms[options.room.name]) {
                throw new Error('ServerChannel.connectBot: options.room.name ' +
                                ...
                                options.room.name + formatChanName(this));
            }
            room = options.room;
        }
        else {
        ...
        throw new Error('ServerChannel.connectBot: options.room is undefined ' +
                        'and no waiting room is found.' + formatChanName(this));
    }
    else {
    ...
    // Check if we are in a level.
    tmp = room.gameLevel ? this.gameInfo.levels[room.gameLevel].clientTypes :
        this.gameInfo.clientTypes;

    if (!tmp[clientType]) {
        throw new Error('ServerChannel.connectBot: client type not found: ' +
                        clientType + formatChanName(this, room));
    }
    ...
    if ('undefined' !== typeof options.id) {
        if ('string' !== typeof options.id) {
            throw new TypeError('ServerChannel.connectBot: options.id ' +
                                'must be string or undefined. Found: ' +
                                options.id + formatChanName(this, room));
        }
        if ('undefined' !== typeof options.replaceId) {
            throw new Error('ServerChannel.connectBot: options.id ' +
                            ...
                            options.replaceId + formatChanName(this, room));
        }

        replacedClient = this.registry.getClient(options.id);
        if (replacedClient) {
            throw new Error('ServerChannel.connectBot: options.id ' +
    ...
    if ('undefined' !== typeof options.start && options.gotoStep) {
        throw new Error('ServerChannel.connectBot: options.start ' +
                        'and options.gotoStep cannot be both set.' +
                        formatChanName(this, room));
    }
...
ServerChannel.prototype.connectPhantom = function(options) {
    ...
    console.log('***Deprecation warn: ServerChannel.connectPhantom no ' +
                'longer functional because phantomjs has been discountinued.');
 ...
 * ### formatChanName
 *
 * Returns a string containing the channel name and level (if any)
 *
 * The returned string begins with a space for concatenation.
 *
 * @param {object} channel The ServerChannel object
 * @param {object} room optional A Room object to check for a level name
 *
 * @return {string} The formatted string
 */
function formatChanName(channel, room) {
    var res;
    res = ' (channel: ' + channel.name;
    if (room && room.gameLevel) res += ', level: ' + room.gameLevel;
    return res + ')';
}
