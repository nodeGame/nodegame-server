 ...
 */
function ServerChannel(sio, options, servernode) {
     ...
     *
     * Copy of the configuration options 
     */
 ...
 *
 * Mixes in default values and user-defined options from the constructor.
 *
 * @see GameServer
 * @see PlayerServer
 * @see AdminServer
 *
 * TODO: accessDeniedUrl is the same for Admin and Server. Maybe it makes
 * more sense to separate completely the conf of server and admin.
 */
ServerChannel.prototype.createServers = function() {
    ...
    adminOptions = {
        ...
        accessDeniedUrl: this.options.accessDeniedUrl
    };
 ...
 * @param {boolean} recon Optional. If TRUE, client must be in
 */
ServerChannel.prototype.placePlayer = function(playerObj, inRoom, recon) {
...
// TODO: make a robust check.

//     sameRoom = fromRoomName !== inRoom;
// 
//     if (!sameRoom & recon) { 
//         throw new Error('ServerChannel.placePlayer: reconnection flag on, ' +
//                         'but new and old room name are different: from "' +
//                         fromRoomName + '" to "' + inRoom + '".');
//     }
//     else if (sameRoom && !recon) {
//         
//     }
    ...
    if (!recon) {
        // If it is a reconnection we do not need to do it.
        // Adds a new room in the room stack in Channel Registry.
