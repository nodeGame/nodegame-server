...
ServerChannel.prototype.connectBot = function(options) {
    ...
    if (options.start && options.gotoStep) {
        throw new Error('ServerChannel.connectBot: options.start ' +
                        'and options.gotoStep cannot be both true. (channel: ' +
                        this.name + ')');
    }
    ...
    node.once('PLAYER_CREATED', function() {
        ...
        // Replace old if, if requested.
        if (replaceId) room.game.matcher.replaceId(replaceId, node.player.id);
        
        // If bot connected to a game room, setup it
        // (unless options say otherwise).
        if (init) {

            room.setupClient(node.player.id);

            if (options.gotoStep) {                
                // This calls the init method of the game (but does not step).
                bot.game.start({ step: false });
                bot.game.gotoStep(options.gotoStep, options.gotoStepOptions);
            }
