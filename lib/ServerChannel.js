...
ServerChannel.prototype.createServers = function() {
    ...
    var tmp, tmpAdmin, tmpPlayer;
    
    // Options are mixed in.
    adminOptions = this.options.admin;
    playerOptions = this.options.player;
    delete this.options.admin;
    delete this.options.player;

    tmp = J.clone(this.options);
    J.mixin(adminOptions, tmp);
    tmp = J.clone(this.options);
    J.mixout(playerOptions, tmp);

    // Enforce a name.
    adminOptions.name = adminOptions.name || '[ADMIN_SERVER]';
    playerOptions.name = adminOptions.name || '[PLAYER_SERVER]';
    
    // Restore the options object.
    this.options.player = tmpPlayer;
    this.options.admin = tmpAdmin;

    // Actually creates the servers.
    this.admin = new AdminServer(adminOptions, this);
    this.player = new PlayerServer(playerOptions, this);
