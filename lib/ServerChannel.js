 ...
 *
 *   - room (Room):
 *     Optional. The room (waiting, game, requirements, etc.)
 *     in which to put the bot. Default: channel.waitingRoom
 *   - clientType (string):
        ...
        if ('string' === typeof options.room) {
            ...
            if (!room) {
                throw new Error('ServerChannel.connectBot: options.room does ' +
                                'not appear to be a registered room. Id: ' +
                                options.room);
            }
        }
        else if ('object' === typeof options.room) {
            if (!this.gameRooms[options.room.id]) {
                throw new Error('ServerChannel.connectBot: options.room does ' +
                                'not appear to be a registered room. Id: ' +
                                options.room.id);
            }
            ...
            throw new TypeError('ServerChannel.connectBot: options.room ' +
                                'must be a Room object, a string with the ' +
                                'id of a Room object, or undefined. Found: ' +
                                options.room);
        }
    }
    else {
