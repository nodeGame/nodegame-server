...
function GameRoom(config) {
    ...
    if (config.playerList && !(config.playerList instanceof PlayerList)) {
        throw new TypeError('GameRoom: config.playerList must be a ' +
     ...
     *
     * TODO: this.playerList is deprecated, use this.clients instead
     */
    this.clients = this.playerList = new ngc.PlayerList();

    // Adding views to distinguish admins and players.
    // Note: this.node.player will be added to admins.
    this.admins = this.clients.view('admin', function(p) { return p.admin ? p.id : undefined; });
    this.players = this.clients.view('player', function(p) { return !p.admin ? p.id : undefined; });

    // Importing initial clients, if any
    this.clients.importDB(config.playerList || config.clients);
