 ...
 *   require statement.
 * @param {boolean} doLogic If TRUE, sets up the logic as well. Default: TRUE.
 * @param {array|string} Optional. clientList List of client IDs to set up,
 *   or one of the following strings:
 *   'all' (all clients), 'players' (all players), 'admins' (all admins).
 *   Default: 'players'.
 * @param {boolean} force If TRUE, skip the check. Default: FALSE.
 *
 * @see GameRoom.runtimeConf
 */
GameRoom.prototype.setupGame = function(mixinConf, doLogic, clientList, force) {
    var game, botGame, channel, node, that, settings;
    var i, clientId, clientObj, games, type;
    var rc;
    ...
    /*
    game = require(this.gamePaths.player)(this, this.treatmentName, settings);

    botGame = null;
    if (this.channel.bots[0]) {
        botGame = require(this.gamePaths.bot)(
                this.channel.bots[0], this, this.treatmentName, settings);
    }
    ...
    */
    ...
    for (i in clientList) {
        if (clientList.hasOwnProperty(i)) {
            ...
            //if (!games.hasOwnProperty(type)) {
                ...
                // Node object for bots?
                //if (type === 'bot') debugger;
                //games[type] = require(this.gamePaths[type])(
                game = require(this.gamePaths[type])(
                        this, this.treatmentName, settings,
                        this.channel.bots[clientId]);

                //if ('object' !== typeof games[type]) {
                if ('object' !== typeof game) {
                    throw new Error('GameRoom.setupGame: room ' + this.name +
                            '. playerPath did not return a valid game object.');
                }
    ...
    /*
    // Clear the state of the clients with 'stop', then set up the actual game.
    if (J.isArray(clientList)) {
        //for (id in ...
        //this.clients.get(id);
        //this.channel.sysLogger.log(
        //    'GameRoom.startGame: game cannot be started.', 'warn');
        node.remoteCommand('stop', clientList);
        node.remoteSetup('nodegame', clientList, game);
    }
    else {
        if (clientList === 'all' || clientList === 'players') {
            this.clients.player.each(function(p) {
                ...
                if (!(p.sid[0] === 'D' && p.sid[1] === 'P')) {
                    ...
                    //if (botGame) node.remoteSetup('nodegame', p.id, botGame);
                    that.channel.bots[0].setup('nodegame', botGame);
                    //that.channel.bots[0].setup('nodegame', that.channel.bots[1]);

                    //if (botGame) setTimeout(function(){node.remoteSetup('nodegame', p.id, botGame);}, 4000);
                    //setTimeout(function(){console.log('***** STARTING');that.channel.bots[0].game.start();}, 4000);
                }
            });
        }
 ...
 *
 * @param {boolean} doLogic If TRUE, sends a start command to the logic
 *   as well. Default: TRUE.
 * @param {array|string} clientList List of client IDs to which to send the
 *   start command, or one of the following strings:
 *   'all' (all clients), 'players' (all players), 'admins' (all admins).
 *   Default: 'all'.
 * @param {boolean} force If TRUE, skip the startable check. Default: FALSE.
 */
 ...
 *
 * @param {boolean} doLogic If TRUE, sends a pause command to the logic
 *   as well. Default: TRUE.
 * @param {array|string} clientList List of client IDs to which to send the
 *   pause command, or one of the following strings:
 *   'all' (all clients), 'players' (all players), 'admins' (all admins).
 *   Default: 'all'.
 * @param {boolean} force If TRUE, skip the pauseable check. Default: FALSE.
 */
 ...
 *
 * @param {boolean} doLogic If TRUE, sends a resume command to the logic
 *   as well. Default: TRUE.
 * @param {array|string} clientList List of client IDs to which to send the
 *   resume command, or one of the following strings:
 *   'all' (all clients), 'players' (all players), 'admins' (all admins).
 *   Default: 'all'.
 * @param {boolean} force If TRUE, skip the resumeable check. Default: FALSE.
 */
 ...
 *
 * @param {boolean} doLogic If TRUE, sends a stop command to the logic
 *   as well. Default: TRUE.
 * @param {array|string} clientList List of client IDs to which to send the
 *   stop command, or one of the following strings:
 *   'all' (all clients), 'players' (all players), 'admins' (all admins).
 *   Default: 'all'.
 * @param {boolean} force If TRUE, skip the stopable check. Default: FALSE.
 */
