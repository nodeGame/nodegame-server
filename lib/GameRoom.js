...
function GameRoom(config) {
     ...
     * ### GameRoom.bots
     *
     * node instances of bots inside the room
     */
    this.bots = [];
}
...
GameRoom.prototype.setupGame = function(mixinConf, doLogic, clientList, force) {
    var game, botGame, channel, node, room, settings;
    var isBot;
    var rc;
    ...
    //botGame = require(this.botPath)(null, this, this.treatmentName, settings);
        ...
        if (clientList === 'all' || clientList === 'players') {
            this.clients.player.each(function(p) {
                ...
                if (!(p.sid[0] === 'D' && p.sid[1] === 'P')) {
                    ...
                    //node.remoteSetup('nodegame', p.id, botGame);
                }
            });
        }
