...
function GameRoom(config) {
     ...
     * ### GameRoom.botPath
     *
     * Bot game client path for the room, or null if not defined.
     */
    this.botPath = this.game.treatments[this.treatmentName].botPath;
 ...
 * @param {array|string} clientList List of client IDs to set up,
...
GameRoom.prototype.setupGame = function(mixinConf, doLogic, clientList, force) {
    ...
    if (this.channel.bots[0]) {
        console.log("WILL REQUIRE", this.channel.bots[0].nodename);
        botGame = require(this.botPath)(this.channel.bots[0], this, this.treatmentName, settings);
    }

    if ('object' !== typeof game) {
        throw new Error('GameRoom.setupGame: room ' + this.name +
                        '. clientPath did not return a valid game object.');
    }
