 ...
 *
 * @param {boolean} startLogic If TRUE, sends a start command to the logic
 ...
 */
GameRoom.prototype.startGame = function(startLogic, clientList, force) {
    var node;
    var i, cid;
    ...
    // TODO: separate this
    if ('boolean' !== typeof startLogic && 'undefined' !== typeof startLogic) {
        throw new TypeError('GameRoom.startGame: ' +
                            'startLogic must be boolean or undefined.');
    }

    if (!J.isArray(clientList) && 'string' !== typeof clientList &&
            'undefined' !== typeof clientList) {

        throw new TypeError('GameRoom.startGame: ' +
                            'clientList must be array, string or undefined.');
    }

    if ('boolean' !== typeof force && 'undefined' !== typeof force) {
        throw new TypeError('GameRoom.startGame: ' +
                            'force must be boolean or undefined.');
    }

    // Apply defaults.
    if ('undefined' === typeof startLogic) startLogic = true;
    ...
    if (J.isArray(clientList)) {
        node.remoteCommand('start', clientList);
    }
    else {
