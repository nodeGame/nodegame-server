...
GameRoom.prototype.setupGame = function(mixinConf, doLogic, clientList, force) {
    ...
    if (doLogic) {
        // Instantiates Logic.
        game = this.clientTypes.logic(node, channel, this,
                                      this.treatmentName, settings);
    ...
    for (i in clientList) {
        if (clientList.hasOwnProperty(i)) {
            ...
            if (!this.clientTypes.hasOwnProperty(type)) {
                throw new Error("GameRoom.setupGame: no path defined for " +
                                "client type '" + type + "'.");
            }
            gameNode = this.channel.bots[clientId];
            game = this.clientTypes[type](this, this.treatmentName,
                                          settings, gameNode);
