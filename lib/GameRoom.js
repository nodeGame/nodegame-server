...
function GameRoom(config) {
     ...
     *
     * Client types defined by the game
     */
     ...
     */
    this.runtimeConf = config.runtimeConf || null;
     ...
     * @see GameRoom.setupGame
     */
    this.gameTreatment = this.runtimeConf ?
        J.mixin(J.clone(this.game.treatments[this.treatmentName]),
                this.runtimeConf)
        : this.game.treatments[this.treatmentName];
...
GameRoom.prototype.getClientType = function(type, mixinConf) {
    ...
    // TODO: HERE CHECK IF NODE IS FINE.

    game = this.clientTypes[type](this.treatmentName, this.gameTreatment,
                                  this.game.stager, this.game.setup,
                                  this.node, this);
...
function setupLogic(mixinConf) {
    ...
    if (!node.socket.isConnected()) {
        node.socket.setSocketType('SocketDirect', {
            socket: this.channel.admin.socketManager.sockets.direct
        });
        node.connect(null, {
