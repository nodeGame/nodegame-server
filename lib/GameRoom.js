...
function GameRoom(config) {
    ...
    // TODO: handle treatments in levels.
    ...
    node.on.preconnect(function(p) {
        ...
        if (!that.allowLateReconnections) {
        ...
        if (reconCb) {
            ...
            if (res === false) {
                ...
                // TODO: handle disposal of unauthorized player correctly.
                node.remoteAlert('Reconnection to game not authorized.');
                return;
            }
        }
        ...
        // If we are still missing more players,
        // pause reconnecting player.
...
GameRoom.prototype.getClientType = function(type, mixinConf) {
    ...
    // TODO: place these functions somewhere and not create them here.

    function wrongNumOfPlayers(type, customCb) {
        var node;
        var waitTime, disconnectStr, cb;
        node = this.node;

        console.log('WWWWWWWWWWRONG')
        ...
        disconnectStr = 'One or more players disconnected. If they ' +
        ...
        that.countdown = setTimeout(function() {
            ...
            node.game.resume();
            if (customCb) customCb.call(node.game);

        }, waitTime * 1000);
    }

    function correctNumOfPlayesAgain(type, customCb) {
        console.log('AAAAAAAAAAAAGAIN.')
        var node;
        node = this.node;
        if (type === 'min') {
