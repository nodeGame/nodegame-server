...
GameRouter.prototype.addRoutes = function(gameDir, gameName, gameInfo, alias) {
    ...
    var basepath, rootDir, monitorDir, publicDir;
    var monitorUri;
    var i, len;
    var name;
    ...
    if (gameInfo.auth && gameInfo.auth.enabled) {
        ...
        // getcode.
        if (gameInfo.auth.getcode) {

            app.get(name + '/getcode/*', function(req, res, next) {
                ...
                // res.status(200).send(
                //req.query.callback + '(' + JSON.stringify({ ciao: 1 }) + ')');
                ...
                if ('function' === typeof gameInfo.auth.getcode) {
                    valid = gameInfo.auth.getcode.validate(userId);
                    if (valid !== true) {
                        res.status(400).json({
                            err: valid
                        });
                ...
                code = channel.registry.tagNext(userId);
                if (code) {
                    res.status(200).json({
                        code: code.id,
                        status: code.valid
                    ...
                    console.log('Oh! No more codes!. Deal with it.');
                }
