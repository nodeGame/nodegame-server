...
const unauthMsg = 'Sorry, but your connection is ' +
    ...
    if (gameInfo.auth && gameInfo.auth.enabled) {
        ...
        app.get(name + '/auth/*', function(req, res, next) {
            ...
            if (auth.mode === 'external') {
                ...
                if (!client) channel.registry.addClient(userId, { pwd: pwd });
            }
...
function renderTemplate(resourceManager, req, res, channel,
    ...
    var context, cb;
    var decoded, room, treatmentName;
    ...
    if (req.cookies && req.cookies.nodegame_token) {
        decoded = verifyToken(req.cookies.nodegame_token, channel.secret);
        if (decoded === false) {
            ...
            room = channel.registry.getClientRoom(decoded.clientId);
