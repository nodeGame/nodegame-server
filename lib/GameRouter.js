...
GameRouter.prototype.addRoutes = function(alias) {
    ...
    // Connect string.
    app.get(name + '/connect.js', function(req, res) {
        let endpoint = channel.playerServer.endpoint;
    ...
    if (gameInfo.auth && gameInfo.auth.enabled) {
        ...
        if (auth.mode !== 'monitor') {
            ...
            // First check authorization and set cookies.

            // Authorization for players.
            ...
            // If not alias, add monitor route.
            // Cookie-checking must include monitor cookie and player cookie.
            if (!alias) {
            ...
            // Cookie-checking includes only player cookie.
            else {
                app.get(name + '/*', function(req, res, next) {
    ...
    if (!alias) {
        ...
        let monitorUri = name + '/monitor/index.htm';

        // let monitorUri = name + '/monitor/';
        ...
        app.get(name + '/monitor/data/*', function(req, res, next) {
            ...
            serveFiles(req, res, files, gameInfo,
        ...
        // Connect endpoint for monitor.
        app.post(name + '/monitor/endpoint', function(req, res) {
            let endpoint = channel.adminServer.endpoint;
            res.send(endpoint);
        });
        ...
        app.get(name + '/monitor/*', function(req, res, next) {
            ...
            // Build path to file.
            let filePath = path.join(monitorDir, file);
    ...
    app.get(name + '/*', function(req, res) {
        ...
        doNoAuthCookie(req, res, channel, tokens, gameInfo);
...
function serveFileOrDie(req, res, gameInfo, cb) {
    ...
    serveFiles(req, res, [file], gameInfo, cb);
}

function serveFiles(req, res, files, gameInfo, cb) {
 ...
 * @param {object} gameInfo The info about the game
 */
function do404(req, res, gameInfo) {
    let file = gameInfo.channel.page404;
    if (file) {
        res.sendFile(path.join(gameInfo.dir, 'public', file));
    }
    else {
...
function doNoAuthCookie(req, res, channel, tokens, gameInfo) {
    ...
    if (gameInfo.channel.noAuthCookie) {
        ...
        if (req.cookies) {
            ...
            // not correct, generates a new ID and sets the cookie.
            if (!decoded || (decoded.session !== channel.session)) {
                let clientId = channel.registry.generateClientId();
                let token = createToken(clientId, channel, tokens);
                res.cookie('nodegame_token', token, {
                    path: '/',
                    httpOnly: true
                });
