...
GameRouter.prototype.addRoutes = function(gameDir, gameName, gameInfo) {
    ...
    if (gameInfo.auth && gameInfo.auth.enabled) {
        ...
        app.get('/' + gameName + '/monitor/auth/:id/:pwd',
                function(req, res, next) {

            var userId, pwd;
            userId = req.params.id;
            pwd = req.params.pwd;
            loginSuper(servernode, gameName, userId, pwd, res, req);
        });

        app.get('/' + gameName + '/auth/:id/:pwd', function(req, res, next) {
            var userId, pwd;
            userId = req.params.id;
            pwd = req.params.pwd;
            login(servernode, gameName, userId, pwd, res, req);
        });
...
function loginSuper(servernode, gameName, user, pwd, res, req) {
    var channel, authorized, token;
...
function login(servernode, gameName, user, pwd, res, req) {
    var channel, authorized, token;
...
function createToken(user, channel, tokenList) {
