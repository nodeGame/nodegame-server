...
WaitingRoom.treatmentCallbacks = {
    ...
    // Important! If names change, change also routine to collect treatment
    // names in WaitingRoom.availableTreatments.
 ...
 *
 * If no bots are necessary, it just dispatches.
 *
 * @see WaitingRoom.dispatch
 */
WaitingRoom.prototype.dispatchWithBots = function() {
    var neededBots, botOptions, dispatchList;
    var i;
    ...
    neededBots = this.POOL_SIZE - dispatchList.length;

    // If no bots are necessary (for example when pool size is 1
    // and ALLOW_SELECT_TREATMENT is true), just dispatch.
    if (neededBots < 1) {
        this.dispatch();
        return;
    }
