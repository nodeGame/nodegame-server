...
WaitingRoom.prototype.parseSettings = function(settings) {
    ...
    if ('undefined' !== typeof settings.ON_CLOSE) {
        if ('function' !== typeof settings.ON_CLOSE) {
            new TypeError(where +
                          'ON_CLOSE must be function or undefined. Found: ' +
                          settings.ON_CLOSE);
        }
        this.ON_CLOSE = settings.ON_CLOSE;
    }

    if ('undefined' !== typeof settings.ON_OPEN) {
        if ('function' !== typeof settings.ON_OPEN) {
            new TypeError(where +
                          'ON_OPEN must be function or undefined. Found: ' +
                          settings.ON_OPEN);
        }
...
WaitingRoom.prototype.closeRoom = function() {
    var len;
    this.roomOpen = false;
    console.log('ROOM is going to be closed.', this.ON_CLOSE);
    ...
    cb = function() {
        ...
        if (that.suspendedPings.length > 0) {
            ...
            if (!that.shouldDispatchMoreGames()) {
                console.log('Warning: room is closed. Aborting previously ' +
                            'suspended pings.');
                that.suspendedPings = [];
            }
            else if (that.clients.player.size() < that.POOL_SIZE) {
                console.log('Warning: not enough players any longer. ' +
