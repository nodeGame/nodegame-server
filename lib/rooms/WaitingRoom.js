 ...
 * ## setupRoom
 *
 * Setup the game after assigning a treatment if a new room is needed or if this
 * is the first dispatch.
 *
 * @param {object} that The WaitingRoom object
 * @param {number} gameNumber The index of the current game to be dispatched
 * @param {array} group Array contains the ids of players in the group
 */
function setupRoom(that, gameNumber, group) {
    var treatmentName, gameRoom, channel, j, groupSize;
    channel = that.channel;
    groupSize = that.GROUP_SIZE;
    // If this is the first dispatch or we need to create a new room
    // anyway, assign a treatment and then setup the game.
    if (!that.lastGameRoom || !that.DISPATCH_TO_SAME_ROOM) {

        // Decide treatment.
        treatmentName = that.decideTreatment(that.chosenTreatment,
            gameNumber);

        // Starts a game level or the main game.
        // TODO: should the main game have a ghost level?
        if (that.gameLevel) {
            gameRoom = channel.gameLevels[that.gameLevel]
                .createGameRoom({
                    clients: group,
        ...
        for (; ++j < groupSize;) {
            channel.moveClient(group[j],
                that.lastGameRoom.name,
                that.name);
        }
        // Setup new group of players in the old room.
        that.lastGameRoom.setupGame(undefined, false, group);
        that.lastGameRoom.startGame(false, group, true);
    }
}
...
WaitingRoom.prototype.dispatchWithBots = function() {
    ...
    var i = -1, gameNumber = -1;

    this.PING_BEFORE_DISPATCH = false;

    neededBots = this.GROUP_SIZE - this.numberOfConnectedPlayers;
    dispatchList = this.clients.player.db;

    for (; ++i < dispatchList.length;) {
        // Player id.
        pId = dispatchList[i].id;
        this.channel.sysLogger.log("Dispatching player " + pId + " with bot.");
        ...
        setupRoom(this, gameNumber, [pId]);

        // Define bot options with most recent gameRoom.
        botOptions = {
            room: this.lastGameRoom,
            clientType: 'bot',
            setup: {}
        };
    ...
    doDispatch = function(opts) {
        ...
        for ( ; ++i < nPlayersToDispatch ; ) {
            ...
            if (groupIdx === groupLimit) {
                gameNumber++;
                setupRoom(this, gameNumber, group);
