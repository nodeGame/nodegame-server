...
const PlayerList = require('nodegame-client').PlayerList;

const HostRoom = require('./HostRoom');
...
WaitingRoom.treatmentCallbacks = {
    ...
    // ### latin_square
    treatment_weighted_random: 'Randomly samples treatments based on weights',
    // ### latin_square
...
function WaitingRoom(config) {
     ...
     */
    this.numberOfDispatches = 0;

    /**
     * ### WaitingRoom.numberOfDispatchesByTreatment
     *
     * List that keeps track of how many dispatches per treatment
     */
    this.numberOfDispatchesByTreatment = {};

    /**
    * ### WaitingRoom.ROTATION_OFFSET
    *
    * Non-negative integer offsetting the default rotation algorithm to
    * assign treatments ("treatment_rotate").
    *
    * @see WaitingRoom.decideTreatment
    */
    this.ROTATION_OFFSET = config.ROTATION_OFFSET || 0;
    ...
    * ### WaitingRoom.lastGameRoom
    *
    * Reference to the last created game room for each treatment
    */
    this.lastGameRoomByTreatment = {};
     ...
     *
     * If TRUE, clients are disconnected if not selected for dispatch
     */
    this.DISCONNECT_IF_NOT_SELECTED = null;

    /**
     * ### WaitingRoom.NOTIFY_UPDATES
     *
     * If TRUE, clients are notified about some updates
     *
     * Updates:
     * - changes in num of players (connections/disconnections)
     * - not being selected for a dispatch
     *
     * Default: TRUE
     */
    ...
    /**
     * ### WaitingRoom.OVERWRITE_DISPATCH_GROUP
     *
     * If set, it overwrites the group of players for the next dispatch only
     *
     * It is set to null at every dispatch.
     *
     * @see WaitingRoom.ALLOW_SELECT_TREATMENT
     */
    this.OVERWRITE_DISPATCH_GROUP = null;
    ...
    /**
     * ### WaitingRoom.ALLOW_USER_DISPATCH
     *
     * If TRUE, users can trigger dispatch from waiting room
     *
     * @see ALLOW_USER_DISPATCH
     */
    this.ALLOW_USER_DISPATCH = false;

    /**
     * ### WaitingRoom.ALLOW_SELECT_TREATMENT
     *
     * If TRUE, users can select the treatment for the dispatch
     *
     * @see ALLOW_USER_DISPATCH
     */
    ...
    /**
     * ### WaitingRoom.REMOTE_DISPATCH
     *
     * If not falsy, it contains information for a remote dispatch
     *
     * Format:
     *
     * ```js
     ...
     *   addTreatment: true,
     *   preprocess: function (url, treatmentName, group, gameRoom) {
     *       // url includes query string if addTreatment is true.
     *       // Modify url.
     *       return url;
     * }
     * ```
     */
    this.REMOTE_DISPATCH = false;
    ...
    /**
     * ### WaitingRoom.ALLOW_QUERYSTRING_TREATMENT
     *
     ...
     *
     * @see WaitingRoom.QUERYSTRING_TREATMENT_VAR
     */
    this.ALLOW_QUERYSTRING_TREATMENT = false;

    /**
     * ### WaitingRoom.QUERYSTRING_TREATMENT_VAR
     *
     * The querystring variable to look for when starting a game via querystring
     *
     * @see WaitingRoom.ALLOW_QUERYSTRING_TREATMENT
     */
    this.QUERYSTRING_TREATMENT_VAR = "treat";

    /**
     * ### WaitingRoom.TREATMENT_TILES
     *
     ...
     * `ALLOW_SELECT_TREATMENT` to be TRUE.
     */
    this.ADD_DEFAULT_TREATMENTS = false;

    /**
     * ### WaitingRoom.TREATMENT_TILE_CB
     *
     * A callback sent to the client that displays the treatments
     *
     * Format:
     *
     * ```js
     * function (treatment, description, index, waitroomWidget) {
     *     return treatment + ": " + description;
     * }
     * ```
     */
    this.TREATMENT_TILE_CB = null;

    /**
     * ### WaitingRoom.userDefinedTreatmentsList
     *
     * Lists user-defined treatments
     *
     * Filled in when availableTreatments is created.
     *
     * @see WaitingRoom.decideTreatment
     */
    this.userDefinedTreatmentsList = [];

    /**
     * ### WaitingRoom.userDefinedTreatmentsMap
     *
     * Maps user-defined treatments to their description
     *
     * Filled in when availableTreatments is created.
     *
     * @see WaitingRoom.decideTreatment
     */
    this.userDefinedTreatmentsMap = {};

    /**
     * ### WaitingRoom.availableTreatments
     *
     * Maps all treatments, including treatment callbacks, to their description
     *
     * Hard-cloned from WaitingRoom.treatmentCallbacks
     */
    this.availableTreatments = {
        treatment_random:
            WaitingRoom.treatmentCallbacks['treatment_random'],
        treatment_weighted_random:
            WaitingRoom.treatmentCallbacks['treatment_weighted_random'],
        treatment_rotate:
            WaitingRoom.treatmentCallbacks['treatment_rotate'],
        treatment_latin_square:
            WaitingRoom.treatmentCallbacks['treatment_latin_square']
    };

    // Fill in the treatment data.
    (function(that) {
        let t;
        if (that.gameLevel) {
        ...
        for (let key in t) {
            if (t.hasOwnProperty(key)) {
                let descr = t[key].description || 'no description';
                // Build the list and map of user-defined treatments.
                that.userDefinedTreatmentsMap[key] = descr;
                that.userDefinedTreatmentsList.push(key);
                // Also complete all available treatments.
                that.availableTreatments[key] = descr;
            }
        }
    })(this);

    /**
     * ### WaitingRoom.weightedTreatments
     *
     * Array of indexes to treatment built according to weights
     */
    this.weightedTreatments = null;

    /**
     * ### WaitingRoom.treatmentWeights
     *
     * Weights for every treatment
     */
     ...
     * ### WaitingRoom.weightedTreatments
     *
     * List of max number of treatments to be launched by this waitroom
     */
    this.treatmentQuotas = null;
...
WaitingRoom.prototype.dispatchWithBots = function(justConnect) {
    ...
    if (this.OVERWRITE_DISPATCH_GROUP) {
    ...
    neededBots = this.POOL_SIZE - pList.db.length;
...
WaitingRoom.prototype.parseSettings = function(settings) {
    ...
    if ('undefined' !== typeof settings.NOTIFY_INTERVAL) {
        if ('number' === typeof settings.NOTIFY_INTERVAL) {
            checkPositiveInteger(settings, 'NOTIFY_INTERVAL', where);
    ...
    if ('undefined' !== typeof settings.NOTIFY_UPDATES) {
        if ('boolean' === typeof settings.NOTIFY_UPDATES) {
            this.NOTIFY_UPDATES =
                settings.NOTIFY_UPDATES;
        }
        else {
            throw new TypeError(where + 'NOTIFY_UPDATES must ' +
    ...
    if ('undefined' !== typeof settings.ROTATION_OFFSET) {
        if ('number' === typeof settings.ROTATION_OFFSET) {
            checkPositiveInteger(settings, 'ROTATION_OFFSET', where);
            this.ROTATION_OFFSET = settings.ROTATION_OFFSET;
        }
        else {
            throw new TypeError(where + 'ROTATION_OFFSET must be a positive ' +
                                'number or undefined. Found: ' +
                                settings.ROTATION_OFFSET);
        }
    }

    if (this.CHOSEN_TREATMENT === 'treatment_weighted_random') {
        if ('object' !== typeof settings.TREATMENT_WEIGHTS) {
            throw new TypeError(where + 'TREATMENT_WEIGHTS must be object ' +
                                'Found: ' + settings.TREATMENT_WEIGHTS);
        }

        this.treatmentWeights = settings.TREATMENT_WEIGHTS;
    }
    ...
    if ('undefined' !== typeof settings.REMOTE_DISPATCH) {
        let obj;
        let r = settings.REMOTE_DISPATCH;
        if ('string' === typeof r) obj = { url: r };
        else if ('object' === typeof r) obj = r;
        if (!obj) {
            throw new TypeError(where + 'REMOTE_DISPATCH must be string, ' +
                               'object, or undefined. Found: ' + r);
        }
        if (!obj.url || 'string' !== typeof obj.url) {
    ...
    if ('undefined' !== typeof settings.ALLOW_PLAY_WITH_BOTS) {
        console.log('***Waiting.room: ALLOW_PLAY_WITH_BOTS is deprecated ' +
                    'use ALLOW_USER_DISPATCH');

        if ('undefined' === typeof settings.ALLOW_USER_DISPATCH) {
            settings.ALLOW_USER_DISPATCH = settings.ALLOW_PLAY_WITH_BOTS;
        }
    }

    if ('undefined' !== typeof settings.ALLOW_USER_DISPATCH) {
        this.ALLOW_USER_DISPATCH = !!settings.ALLOW_USER_DISPATCH;
    }
    ...
    if (this.ALLOW_SELECT_TREATMENT && !this.ALLOW_USER_DISPATCH) {
        this.channel.sysLogger.log('ALLOW_SELECT_TREAMENT is true, but ' +
                                   'ALLOW_USER_DISPATCH is false, so it ' +
                                   'will be ignored', 'warn');

        this.ALLOW_SELECT_TREATMENT = false;
    }

    if ('undefined' !== typeof settings.ALLOW_QUERYSTRING_TREATMENT) {
        this.ALLOW_QUERYSTRING_TREATMENT =
            !!settings.ALLOW_QUERYSTRING_TREATMENT;
    }

    if ('undefined' !== typeof settings.QUERYSTRING_TREATMENT_VAR) {
        this.QUERYSTRING_TREATMENT_VAR = settings.QUERYSTRING_TREATMENT_VAR;
    }

    tmp = settings.TREATMENT_TILE_CB;
    if ('undefined' !== typeof tmp) {
        if ('function' !== typeof tmp) {
            throw new TypeError(where + 'TREATMENT_TILE_CB must ' +
                                'be function. Found: ' + tmp);
        }
        this.TREATMENT_TILE_CB = tmp;
    }

    tmp = settings.TREATMENT_TILES;
    if ('undefined' !== typeof tmp) {
        this.TREATMENT_TILES = !!tmp;
    }

    tmp = settings.ADD_DEFAULT_TREATMENTS;
    if ('undefined' !== typeof tmp) {
        this.ADD_DEFAULT_TREATMENTS = !!tmp;
    }

    this.node.on.data('DISPATCH', function(msg) {
        var pid, t, logStr;

        pid = msg.from;
        // Check if the client was allowed to send it.
        if (!that.ALLOW_USER_DISPATCH && !that.channel.registry.isAdmin(pid)) {
            that.channel.sysLogger.log('Received illegal dispatch ' +
                                       'by client ' + pid, 'warn');
            return;
        }

        logStr = 'Dispatch started by client ' + pid;
        ...
        if (that.GROUP_SIZE === 1) {
            // Simulating a PlayerList object without the need to create one.
            that.OVERWRITE_DISPATCH_GROUP = { db: [ {
                id: msg.from, sid: ''
            } ] };
        }
...
WaitingRoom.prototype.makeWidgetConfig = function() {
    var o = {
        ...
        userCanDispatch: this.ALLOW_USER_DISPATCH,
        userCanSelectTreat: this.ALLOW_SELECT_TREATMENT,
        queryStringDispatch: this.ALLOW_QUERYSTRING_TREATMENT,
        queryStringTreatVar: this.QUERYSTRING_TREATMENT_VAR,
        treatmentTileCb: this.TREATMENT_TILE_CB,
        treatmentTiles: this.TREATMENT_TILES,
        addDefaultTreatments: this.ADD_DEFAULT_TREATMENTS
    };
 ...
 *        If property pList is missing, it pList is automatically added 
 *        and contains the players that have responded to ping; if it is
 *        present, only those players will be pinged and a possibly returned.
...
WaitingRoom.prototype.kickUnresponsivePlayers = function(callback, options) {
    ...
    if (this.isPingInProgress) {
        this.channel.sysLogger.log('Warning: Suspending call to ' + where +
    ...
    // If a pList was passed to dispach, we ping only these players.
    if (options.callbackArgs.pList) {
        players = options.callbackArgs.pList.db;
    }
    else {
...
WaitingRoom.prototype.dispatch = (function() {
    ...
    parseGroupSizeOptions = function(opts) {
        var groupSize, numberOfGames, nPlayers, pList;
        opts = opts || {};
        ...
        if ('undefined' === typeof numberOfGames) {
            // pList can be a simulate object, so we directly check .length.
            nPlayers = pList.db.length;
            if (!nPlayers ||
    ...
    doDispatch = function(opts) {
        ...
        pList = opts.pList || this.clients.player;
        // In case the player is participating in a single-player task,
        // that.OVERWRITE_DISPATCH_GROUP = [ msg.from ]; is an array.
        ...
        if (nPlayers > groupSize) {
            ...
            if (this.PLAYER_GROUPING) {
                // Note: OVERWRITE_DISPATCH_GROUP simulates a PlayerList
                // object, but it should never enter here, because it is
                // only for GROUP_SIZE=1.
        ...
        for ( ; ++i < nPlayersToDispatch ; ) {
            ...
            if (groupIdx === groupLimit) {
                ...
                if (this.REMOTE_DISPATCH) {
                    // Move clients into existing treatment room.
                    let r = this.REMOTE_DISPATCH;
                    let url = r.url;
                    if (r.addTreatment) {
                        url += (~url.indexOf('?')) ?  '&' : '?';
                        url += `t=${treatmentName}`;
                    ...
                    if (r.preprocess) {
                        url = r.preprocess(url, treatmentName, group, this);
                    }
                    this.node.redirect(url, group);
                }
                else {

                    // If this is the first dispatch or we need to create a new
                    // room anyway, assign a treatment and then setup the game.
                    if (!this.lastGameRoomByTreatment[treatmentName] ||
                        !this.DISPATCH_TO_SAME_ROOM) {
                        ...
                        // will automatically start them (if syncStepping).
                        gameRoom.startGame(true, []);

                        // Store references.
                        // TODO: check if we need lastGameRoom (legacy).
                        this.lastGameRoom = gameRoom;
                        this.lastGameRoomByTreatment[treatmentName] = gameRoom;
                    }
                    else {
                        let treatmentRoom =
                            this.lastGameRoomByTreatment[treatmentName];

                            // console.log('ROOM N: ', treatmentRoom.name);
                            // console.log('ROOM T: ',
                            //             treatmentRoom.treatmentName);
                        ...
                        treatmentRoom.setupGame(undefined, false, group);
                        treatmentRoom.startGame(false, group, true);
                    }
                }
                ...
                // Keep count of which treatment was dispatched.
                this.numberOfDispatchesByTreatment[treatmentName] =
                    this.numberOfDispatchesByTreatment[treatmentName] ?
                    ++this.numberOfDispatchesByTreatment[treatmentName] : 1;
        ...
        if (closeAfterDispatch) {
            // Ste: commented this line, because it is set at the very end.
        ...
        if (this.NOTIFY_UPDATES) {
            ...
            for (i = 0; i < nPlayers; ++i) {
                ...
                if (pList.db[i].timesNotSelected) {
                    ++pList.db[i].timesNotSelected;
                }
                else {
                  pList.db[i].timesNotSelected = 1;
                }
    ...
    return function(opts) {
        ...
        // Must come before parseGroupSizeOptions.
        // If a group was set, copy it into settings and delete it.
        if (this.OVERWRITE_DISPATCH_GROUP) {
            opts.pList = this.OVERWRITE_DISPATCH_GROUP;
            this.OVERWRITE_DISPATCH_GROUP = null;
        }
 ...
 *
 * @param {mixed} t Optional String, object, function to decide a treatment
 *
 * @return {string} The chosen treatment
 */
WaitingRoom.prototype.decideTreatment = function(t, idxInBatch) {
    let treatments = this.userDefinedTreatmentsList;

    // treatments = this.checkQuotas(treatments);

    let tLen = treatments.length;

    if (t === 'treatment_rotate') {
        return treatments[(this.numberOfDispatches +
                           this.ROTATION_OFFSET) % tLen];
    }
    if (t === 'treatment_latin_square') {
        if (!this.latinSquare) {
            // Create a long array concatening all the latin square rows.
            // TODO: make the concatenation a feature of J.latinSquare.
            // TODO: make the latin square already with array names (feature).
            this.latinSquare = [];
            let lq = J.latinSquare(tLen);
            lq.forEach(ar => this.latinSquare = [ ...this.latinSquare, ...ar ]);
        }
        tLen = this.latinSquare.length;
        let idx = this.latinSquare[(this.numberOfDispatches +
                                   this.ROTATION_OFFSET) % tLen];

        // console.log(this.latinSquare);
        // console.log(idx);
        return treatments[idx];
    }
    if (t === 'treatment_weighted_random' && !this.treatmentWeights) {
        t = 'treatment_random';
        this.channel.sysLogger.log('WaitingRoom.decideTreatment: ' +
            'treatment_weighted_random selected but no weights found. Using ' +
            'equal weights', 'warn');
    }
    else if (t === 'treatment_weighted_random') {

        if (!this.weightedTreatments) {
            // Expand the treatments array according to weights.
            this.weightedTreatments = [];
            // Sum of weights (used to normalize weights).
            let sum = 0;

            // Treatments not specified in the weights array, divide equally
            // remainder of 1 - sum of weights.
            let leftOverTreatments = [];

            treatments.forEach((t, idx) => {

                let w = this.treatmentWeights[t];
                if ('undefined' === typeof w) {
                    leftOverTreatments.push(t);
                }
                else {
                    if (false === J.isNumber(w, 0, null, true)) {
                        throw new TypeError('WaitingRoom.decideTreatment: ' +
                                            'invalid treatment weight for ' +
                                            t + ': ' + w);
                    }
                    sum += w;
            ...
            if (sum === 0) {
                throw new Error('WaitingRoom.decideTreatment: sum of ' +
                                'weights is zero');
            }

            if (sum < 1 && leftOverTreatments.length) {

                // Leftover from sum.
                let l = 1 - sum;

                // Divide leftover weight across other treatments.
                leftOverTreatments.forEach((t, idx) => {
                    this.treatmentWeights[t] = l / leftOverTreatments.length;
                });
            ...
            // For every treatment compute the cumulative density function.
            let cdf = 0;
            treatments.forEach((t, idx) => {
                // Normalize weight.
                let w = this.treatmentWeights[t];
                // Not found means zero weights.
                w = w ? w / sum : 0;
                cdf += w;
                this.weightedTreatments[idx] = cdf;
                // Debug.
                console.log(t, w, cdf)
                console.log(this.weightedTreatments);
                console.log('-----------------------')
            });

            // Force the last one to be 1.
            this.weightedTreatments[this.weightedTreatments.length - 1] = 1;
        }

        // Select a random treatment with weight, searching in cdf.
        let idx = 0;
        let rnd = Math.random();
        for (; idx < this.weightedTreatments.length; idx++) {
            if (rnd <= this.weightedTreatments[idx]) break;
        }
    ...
    if ('function' === typeof t) {
        ...
        t = t(treatments, this.channel.autoRoomNo, idxInBatch,
              this.numberOfDispatches);

        if ('string' !== typeof t) {
            throw new TypeError('WaitingRoom.decideTreatment: callback must ' +
                                'return string. Found: ' + t);
        }
        if (!this.userDefinedTreatmentsMap[t])  {
            throw new TypeError('WaitingRoom.decideTreatment: callback ' +
                                'returned unknown treatment: ' + t);
        }
    }
...
//  */
// WaitingRoom.prototype.checkQuotas = function(treatments) {
//     if (!this.treatmentQuotas) return treatments;
//     // Make a copy of array.
//     let t = treatments.slice(0);
//     if ('function' === this.treatmentQuotas) {
//         t = this.treatmentQuotas(this.numberOfDispatchesByTreatment, t);
//     }
//     else if ('object' === typeof this.treatmentQuotas) {
//
//     }
//
// };
