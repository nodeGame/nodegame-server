...
/**
 * ## WaitingRoom.connectBots
 *
 * Dispatches all waiting players to new gamerooms and connects bot to each 
 * room
 * 
 */
WaitingRoom.prototype.connectBots = function() {
    ...
    for (; ++i < dispatchList.length;) {
        ...
        console.log("Connecting player " + pId + " to bot");
        ...
        if (!this.lastGameRoom || !this.DISPATCH_TO_SAME_ROOM) {
            ...
            // Decide treatment.
            treatmentName = this.decideTreatment(this.chosenTreatment,
                gameNumber);

            // Starts a game level
            gameRoom = this.channel.gameLevels[this.gameLevel]
                .createGameRoom({
                    clients: [pId],
...
WaitingRoom.prototype.parseSettings = function(settings) {
    ...
    if ('undefined' != typeof settings.PLAY_WITH_BOT_OPTION) {
        this.PLAY_WITH_BOT_OPTION = settings.PLAY_WITH_BOT_OPTION;
    }
    else {
        this.PLAY_WITH_BOT_OPTION = false;
    }

    if (this.PLAY_WITH_BOT_OPTION) {
        this.node.on.data("PLAYWITHBOT", function (msg){
            var pid = msg.id;
            console.log("Bot connections started by player " + pid);
            that.connectBots();
        });
    }
...
WaitingRoom.prototype.makeWidgetConfig = function() {
    return {
        ...
        playWithBotOption: this.PLAY_WITH_BOT_OPTION
    };
};
