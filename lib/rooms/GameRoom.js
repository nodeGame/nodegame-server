...
function GameRoom(config) {
     ...
     * @see Room.log
     */
    this.logFilePath = path.resolve(this.channel.getGameDir(),
                                    'logs', this.name + '.csv');
    ...
    node.on.pconnect(function(p) {
        ...
        if (true) that.log(J.now() + ',"connect","' + p.id + '"');
    });
    ...
    node.on.pdisconnect(function(p) {
        console.log('Disconnection: ' + p.id + ' (server stage: ' +
                    node.player.stage + ')');
        // TODO: get this setting.
        if (true) that.log(J.now() + ',"disconnect","' + p.id + '"');
    });
    ...
    node.on.preconnect(function(p) {
        ...
        // Logs reconnections.
        // TODO: get this setting.
        if (true) that.log(J.now() + ',"reconnect","' + p.id + '"');
 ...
 *
 * Computes, sends, dumps the total bonus of each player
 *
 ...
 *   - cb: (undefined) If set, can manipulate the bonus object before sending it
