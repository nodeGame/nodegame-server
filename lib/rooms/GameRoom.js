...
function GameRoom(config) {
    ...
    node.on.preconnect(function(p) {
        ...
        if (code.stageLevel === DONE) reconOptions.willBeDone = true;
        ...
        if (!reconOptions.targetStep) {
            ...
            if (compareSteps > 0) reconOptions.targetStep = code.stage;
            else reconOptions.targetStep = curStage;
        }

        console.log('************', reconOptions.targetStep);

        if (reconOptions.targetStep === code.stage && compareSteps === 1) {
        }
