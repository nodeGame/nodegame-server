...
function GameRoom(config) {
    ...
    node.on.pdisconnect(function(p) {
        console.log('Disconnection in stage: ' + p.id + ' (server stage: ' +
                    node.player.stage + ')');
    });
 ...
 *
 * Computes the total bonus of each player
 *
 * Optionally, sends a message with current bonus to each player.
 * Optionally, saves the results in a bonus file in csv format.
 *
 * @return {array} The database of the logic
 *
 * @see GameRoom.node
...
GameRoom.prototype.computeBonus = function(options) {
    ...
    out = this.clients.player.map(function(p) {
        var client, accesscode, exitcode;
        ...
        if (exchangeRate !== 1) client[winProperty] *= (exchangeRate);

        if (winDecimals !== -1) {
            client[winProperty].toFixed(2);
            client[winProperty] = parseFloat(client[winProperty], 10);
        }

        if (options.say) {
            node.say('WIN', p.id, {
                win: client[winProperty],
                exitclient: client.ExitCode
            });
        }

        if (options.print) console.log(p.id, ':',
                                       client[winProperty], client.ExitCode);

        out = [p.id, client.ExitCode || missing, client[winProperty] ]

        // Execute callback on row.
        if (options.cb) options.cb(out, p);

        if (options.dump) outStr += out.join(', ') + '\n';

        return out;
    });

    if (options.dump) {
        filename = node.game.memory.getWD() + 'bonus.csv';
        bonusFile = fs.createWriteStream(filename);
        bonusFile.on('error', function(err) {
            console.log('Error while saving bonus file: ', err);
        });
        bonusFile.write(["access", "exit", "bonus"].join(', ') + '\n');
        bonusFile.write(outStr);
        bonusFile.end();
    }
...
function setupLogic(mixinConf) {
    ...
    node.game.memory.setWD(path.resolve(this.channel.getGameDir(), 'data'));
