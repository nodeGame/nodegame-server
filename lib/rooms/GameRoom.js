...
GameRoom.prototype.use = function(policies = {}) {
    ...
    for (let policy in policies) {
        if (policies.hasOwnProperty(policy)) {
            ...
            let value = policies[policy];

            if (policy === "defaultStreams") {
                defaultStreamsPolicy(this, value);
            }
            else if (policy === "singlePlayerEndGame") {
...
function writeWithBk(filename, append, bk, logger, str, header) {
    ...
    if (bk === false) {
        ...
        if (fileExists) {
            // TODO: file needs to be unlinked if it exists.
...
function defaultStreamsPolicy(gameRoom, mask = {}) {
    ...
    // Done.
    if (mask === true || mask.memory === true) memory.stream();

    // Feedback.
    if (mask === true || mask.feedback === true) {
