 ...
 * @see GameRoom.node
 */
GameRoom.prototype.computeBonus = function() {
    var node, channel;
    var out, property;
    ...
    out = clients.player.map(function(p) {
        var client, accesscode, exitcode;
        client = channel.registry.getClient(p.id);

        accesscode = code.AccessCode;
        exitcode = code.ExitCode;
        code.win = Number((code[property] || 0) * (exchangeRate)).toFixed(2);
        ...
        out = [p.id, code.ExitCode || 'na', code.win,
