...
GameRoom.prototype.computeBonus = function(options) {
    ...
    if ('undefined' !== typeof options.header) {
        ...
        header = [ 'id', 'access', 'exit', 'type', 'bonus' ];
        headerKeys = [
            'id', 'AccessCode', 'ExitCode', 'clientType', winProperty
        ];
        ...
        if (options.addDisconnected) {
            ...
            headerKeys.push('disconnectedStage');
        }
        // END MODS.
    }
...
function formatField(obj, field, missing) {
    let res = 'undefined' !== typeof obj[field] ? obj[field] : missing;
    // True into 1.
    if (res === true) res = 1;
    // Falsy into 0.
