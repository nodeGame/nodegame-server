...
const isWin = require('os').platform().charAt(0) === 'w';
...
GameRoom.prototype.updateBonus = function(id, update, opts = {}) {
    ...
    if ('string' !== typeof id) {
        throw new TypeError('GameRoom.updateBonus: id must be string. ' +
                            'Found: ' + id);
    }
    let client = this.channel.registry.getClient(id);
    if (!client) return false;
    if ('string' === typeof update) update = J.isNumber(update);
    if ('number' !== typeof update) {
        throw new TypeError('GameRoom.updateBonus: update must be number. ' +
                            'Found: ' + origUpdate);
    }
...
GameRoom.prototype.use = function(policies = {}) {
    ...
    for (let p in policies) {
        if (policies.hasOwnProperty(p)) {
            ...
            if (p === "defaultStreams") {
                let r = defaultStreamsPolicy(this, value);
                res = J.mixin(res, r);
            }
            else if (p === "initSinglePlayer") {
                let r = initSinglePlayerPolicy(this, value);
                res = J.mixin(res, r);
            }
            else if (p === "initMultiPlayer") {
                let r = initMultiPlayerPolicy(this, value);
                res = J.mixin(res, r);
            }
            else if (p === "singlePlayerEndGame") {
                let r = singlePlayerEndGamePolicy(this, value);
                res = J.mixin(res, r);
            }
            else if (p === "initSurvey") {
                let r = initSurveyPolicy(this, value);
                res = J.mixin(res, r);
            }
            else {
...
const preprocessGame = (gameRoom, clientType, game) => {
    ...
    for (let s in stages) {
        if (stages.hasOwnProperty(s)) {
            let stage = stages[s];
            updateFrameUri(gameRoom, stage);

            stage.steps.forEach((stepId) => {
                let step = game.plot.steps[stepId];
                updateFrameUri(gameRoom, step);
            });
        }
    }
};
 ...
 * ### updateFrameUri
 *
 * Calls findFrame and updates the frame property depending on its type
 *
 * @param {object} gameRoom The gameRoom object
 * @param {object} stepOrStage The object for step/stage
 * 
 * @see findFrame
 */
const updateFrameUri = (gameRoom, stepOrStage) => {
    let frameUri = stepOrStage.frame;
    const isFrameObj = 'object' === typeof frameUri;
    if (isFrameObj) frameUri = frameUri.uri;
    const frame = findFrame(gameRoom, stepOrStage.id);
    if (frame) {
        if (isFrameObj) stepOrStage.frame.uri = frame;
        else stepOrStage.frame = frame;
    }
};
...
const findFrame = (gameRoom, id) => {
    ...
    frame = _findFrame(dir, id);

    // File names are case-insensitive on Windows, but not on Linux.
    if (!frame && !isWin) {
        id = id.toLowerCase();
        frame = _findFrame(dir, id);
    }
...
const _findFrame = (dir, id) => {
    ...
    if (fs.existsSync(path.join(viewsDir, file))) return file;
...
function initSurveyPolicy(gameRoom, mod) {
    ...
    return { surveyWidget, capitalizeInput };
}
