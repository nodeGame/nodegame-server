...
GameRoom.prototype.computeBonus = function(options) {
    ...
    if (options.clients) {
        ...
        // MODS.
        if (options.addDisconnected &&
            this.currentlyDisconnected && this.currentlyDisconnected.length) {

            let d = this.currentlyDisconnected;
            let len = d.length;
            for (let i = 0; i < len; i++) {
                // Add clients if disconnected.
    ...
    clients.forEach(function(p) {
        ...
        // Send msg.
        if (options.say && !p.disconnected) node.say('WIN', p.id, obj);
