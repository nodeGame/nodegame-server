...
var DONE = constants.stageLevels.DONE;
...
GameRoom.prototype.getClientType = function(type, mixinConf, gameNode) {
    ...
    if (type === 'logic') {
        ...
        if ('undefined' === typeof game.verbosity) {
            game.verbosity = constants.verbosity_levels.warn;
        }
    }
 ...
 *
 * @param {string|boolean} The updated value or FALSE if the client is not
 *   not found.
 *
...
GameRoom.prototype.computeBonus = function(options) {
    ...
    var header, headerKeys, headerLen, fileExists, append;
    ...
    clients.forEach(function(p) {
        var client, accessCode, exitCode, obj, tmp, id;
    ...
    if (dump) {
        ...
        append = fileExists && options.append;

        if (options.backup === false) {
            writeBonusFile(filename, header, outStr, channel.sysLogger, append);
        }
        else {
            if (fileExists) {
