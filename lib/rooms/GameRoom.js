...
/**
 * @api experimental
 * ## GameRoom.handleGameEndRequests
 *
 ...
 * @see GameRoom.computeBonus
 */
GameRoom.prototype.handleRequest = function(label, mod) {
    const logger = this.channel.sysLogger;

    // TODO: if more automatic requests are added, create an object to
    // store references to listeners (such as this.gameEndListner).
    ...
    if (label === "WIN") {
        ...
        if (mod) {
            if (this.gameEndListener) {
                ...
                if ('object' === typeof mod) this.handleGameEnd(false);
                // Otherwise nothing to do (already registered).
                else return;
            }

            // Keep reference to remove it, if mod = false.
            this.gameEndListener = node.on.data("WIN", function (msg) {
                let client = this.channel.registry.getClient(msg.from);
                if (!client) {
                ...
                if (client.checkout) {
                    ...
                    if ('object' === typeof mod) opts = J.merge(opts, clients);

                    // Compute total win.
                    this.computeBonus(opts);

                    // Mark client checked out.
                    this.channel.registry.checkOut(msg.from);
                }
            });
        }
        else if (mod === false && this.gameEndListener) {
            node.off(this.gameEndListener);
            this.gameEndListener = null;
        }
        else {
            logger.log("GameRoom.handleGameEndRequests: " +
