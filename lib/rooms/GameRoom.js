...
GameRoom.prototype.computeBonus = function(options) {
    ...
    var winProperty, winDecimals, exchangeRate, missing, header, headerLen;
    var i;
    ...
    out = this.clients.player.map(function(p) {
        var client, accessCode, exitCode, obj;
        ...
        if (options.dump) {
            ...
            if (headerLen == 1) {
                ...
                outStr += ',' + obj[header[0]] + ',' + obj[header[1]];
            }
            else if (headerLen == 3) {
                outStr += ',' + obj[header[0]] + ',' + obj[header[1]] +
                    ',' + obj[header[2]];
    ...
    if (options.dump) {
        ...
        if (options.backup === false) {
            ...
            fs.exists(filename, function(exists) {
                if (exists) {
                    filenameBak = filename + '.' + J.now() + '.bak';
                    fs.copy(filename, filenameBak)
                        .then(function() {
                            writeBonusFile(filename, header, outStr,
                                           channel.sysLogger);
                        })
                        .catch(function(err) {
 ...
 */
function writeBonusFile(filename, header, content, sysLogger) {
    var bonusFile;
