...
GameRoom.prototype.handleRequest = function(label, mod) {
    ...
    if (label === "WIN") {
        ...
        if (mod) {
            ...
            this.gameEndListener = this.node.on.data("WIN", function (msg) {
                let client = that.channel.registry.getClient(msg.from);
                if (!client) {
                ...
                if (client.checkout) {
                    ...
                    // Compute total win.
                    that.computeBonus(opts);

                    // Mark client checked out.
                    that.channel.registry.checkOut(msg.from);
                }
            });
        }
        else if (mod === false && this.gameEndListener) {
            this.node.off(this.gameEndListener);
            this.gameEndListener = null;
        }
        else {
            logger.log("GameRoom.handleGameEndRequests: " +
