...
function GameRoom(config) {
    ...
    (() => {
        ...
        if (gameStages.length === 2) {
            console.log('***depracation warning: game.stages function  ' +
                        'should accept 5 parameters, like client types.');
...
GameRoom.prototype.computeBonus = function(options) {
    var node, channel;
    var clients;
    var out, outStr, prolificStr;
    var filename;
    var basePay, winProperty, winDecimals, exchangeRate, miss;
    var header, headerKeys, headerLen;
    var dump, print, say, amt;
    var i;

    node = this.node;
    channel = this.channel;

    let settings = node.game.settings;

    // Set options.
    options = options || {};

    // Say, dump, print (true by default).
    say = 'undefined' === typeof options.say ? true : options.say;
    dump = 'undefined' === typeof options.dump ? true : options.dump;
    print = 'undefined' === typeof options.print ? true : options.print;
    ...
    if (options.winProperty) {
        ...
        winProperty = options.winProperty;
    }
    else {
        winProperty = 'win';
    }
    ...
    if (options.clients) {
        ...
        // MODS.
        if (options.addDisconnected !== false && this.haveDisconnected) {
    ...
    if (options.headerAdd) {
        header = [ ...header, ...options.headerAdd ];
    }
    ...
    clients.forEach(function(p) {
        ...
        if (options.showBonus === false) obj.showBonus = false;
    ...
    if (dump) {
        ...
        writeWithBk(filename, options.append, options.backup,
...
function writeWithBk(filename, append, bk, logger, str, header) {
    ...
    if (bk === false) {
        writeBonusFile(filename, header, str, logger, append);
    }
    else {
        if (fileExists) {
            let filenameBak = '.' + filename + '.bak';

            fs.copy(filename, filenameBak).then(function() {
                writeBonusFile(filename, header, str, logger, append);
            })
            .catch(function(err) {
                logger.log('An error occurred while creating backup of bonus ' +
                           'file: ' + filename, 'error');
