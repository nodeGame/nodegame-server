...
GameLoader.prototype.addGame = function(gameInfo, gameDir) {
    ...
    this.servernode.info.games[gameName] = {
        ...
        /////////////////
        // Experimental.
        gameStages: gameConf.gameStages
    };
    ...
    // TODO: shall we load everything anyway, just avoid creating rooms and
    // routes?
    if (channel === false) {
        this.servernode.info.games[gameName].disabled = true;
        return false;
    }
    ...
    // in the main settings object and for levels also.
...
GameLoader.prototype.buildLevelConf = function(directory, gameName, level) {
    var levelObj;
    if ('string' !== typeof directory) {
        this.doErr('GameLoader.buildLevelConf: directory must be string', {
            game: gameName,
            level: level,
            found: directory
        });
...
GameLoader.prototype.loadGameDir = function(directory, gameName, level) {
    ...
    // just modifies the stager, so the next call to setup receives
    ...
    if (fs.existsSync(gameSetupPath)) {
        ...
        // TODO2: gameStages is a function.
        const dir = level ? origInfo.dir : directory;
        gameSetup = gameSetup(gameSettings, gameStages, dir, level);
 ...
 *
 * Look up the settings of a game and parses them into a list of treatments
 *
 * If levels are found they are also parsed into treatments. Each level will
 * share the settings and treatments of the game folder.
 *
 * Note! Modifies the values inside the `servernode.info.games` object.
 *
 * @param {string} gameName The name of the game
 * @param {boolean} doLevels Optional. If TRUE levels are also parsed into
 *   treatments. Default: TRUE.
 */
GameLoader.prototype.parseSettings = function(gameName, doLevels) {
    var info, settings, level;
    ...
    if ('undefined' === typeof doLevels || doLevels) {
        // Do levels, if any.
        if (info.levels) {
            for (level in info.levels) {
                if (info.levels.hasOwnProperty(level)) {

                    // Parse settings into treatments.
                    settings = parseGameSettings(info.levels[level].settings);

                    if ('object' !== typeof settings) {
                        this.doErr('GameLoader.parseSettings: an error ' +
                                   'occurred while ' +
                                   'parsing the settings object', {
                                       game: gameName,
                                       level: level,
                    ...
                    // Replace old settings with parsed ones.
                    info.levels[level].settings = settings;
                }
            }
        }
    }
...
GameLoader.prototype.loadChannelDir = function(directory, gameName) {
    var pwdFile, jwtFile, res;
    var channelPath, channel;

    // 1. Channel settings.
    let settings = this.loadChannelFile(directory, gameName);

    // Method addChannel handles the `disabled` flag.
    channel = this.servernode.addChannel(settings);
    // Errors in channel creation are already logged.

    // If no channel was actually created, return.
    ...
    // In 8 credentials are loaded before and channel.credentials.js is ignored.
    if (!channel.credentials) {

        pwdFile = path.join(channelPath, 'channel.credentials.js');
        res = loadSyncAsync(pwdFile, settings, 'loadChannelDir',
            function(credtls) {

            channel._loadingCredentials = null;
            if ('object' !== typeof credtls) {
                throw new TypeError('GameLoader.loadChannelDir: credentials ' +
                                    'callback did no return an object. ' +
                                    'Found: ' + typeof credtls);
            }
            if ('string' !== typeof credtls.user ||
                credtls.user.trim() === '') {
    ...
    // In 8 credentials are loaded before and channel.secret.js is ignored.
    if (!channel.secret) {
        jwtFile = path.join(channelPath, 'channel.secret.js');

        res = loadSyncAsync(jwtFile, settings, 'loadChannelDir',
        function(secret) {
 ...
 *
 * Loads _channel.settings.js_ from file system
 *
...
GameLoader.prototype.loadChannelFile = function(directory, gameName) {
    ...
    const channelsFile = path.join(directory, 'channel', 'channel.settings.js');
    ...
    const channelConf = require(channelsFile);
    ...
    // Load and replace private data if available.

    const privateFile = path.join(directory, 'private', 'channel.json');
    const channelJSONErr = 'GameLoader.loadChannelFile: file private' +
                            path.sep + 'channel.json ';

    let secret, playerEndpoint, adminEndpoint, admin;
    if (fs.existsSync(privateFile)) {

        // TODO: improve checks, re-use those in v7.
        const privateConf = require(privateFile);

        secret = privateConf.secret;
        if (!secret) {
            this.doErr(channelJSONErr + 'secret is undefined',
                    { game: gameName });
            return;
        }

        playerEndpoint = privateConf.playerEndpoint;
        if (!playerEndpoint) {
            this.doErr(channelJSONErr + 'playerEndpoint is undefined',
                    { game: gameName });
            return;
        }

        adminEndpoint = privateConf.adminEndpoint;
        if (!adminEndpoint) {
            this.doErr(channelJSONErr + 'adminEndpoint is undefined',
                    { game: gameName });
            return;
        }

        let admins = privateConf.admins;
        if (!admins || admins.length === 0) {
            this.doErr(channelJSONErr + 'admins is undefined or an empty array',
                    { game: gameName });
            return;
        }
        ...
        if ('string' === typeof channelConf.playerServer) {
            channelConf.playerServer = playerEndpoint;
        }
        else {
            if (!channelConf.playerServer) channelConf.playerServer = {};
            channelConf.playerServer.endpoint = playerEndpoint;
        }

        if ('string' === typeof channelConf.adminServer) {
            channelConf.adminServer = adminEndpoint;
        }
        else {
            if (!channelConf.adminServer) channelConf.adminServer = {};
            channelConf.adminServer.endpoint = adminEndpoint;
        }
    }
    else {
        this.doErr(channelJSONErr + 'not found or not readable. Using v7 ' +
                   'loading. Please migrate to v8 channel.json file for ' +
                   'enhanched security. See this link XXX', // TODO.
                   {
                    game: gameName,
                    log: 'warn'
                 });
    ...
    // TODO: check what is this else branch doing?
    else {
        const roomNo = loadRoomNo(dataDir);
    ...
    let page404 =  channelConf.page404 || '404.htm';
    const publicDir = path.join(directory, 'public');
    if (!fs.existsSync(path.join(publicDir, page404))) {
        this.doErr('GameLoader.loadChannelFile: page 404 not in public: ' +
            page404, {
            game: gameName,
            log: 'warn'
        });
        channelConf.page404 = false;
    }
    else {
        channelConf.page404 = page404;
    }
...
GameLoader.prototype.loadAuthDir = function(directory, channel) {
    ...
    if (!fs.existsSync(authSettingsFile)) {
        ...
        if ('object' !== typeof settings) {
            throw new Error('GameLoader.loadAuthDir: invalid settings file, ' +
                            'did not return an object: ' + authSettingsFile);
        }
    }
    ...
    if (generator) {
        ...
        if (!path.isAbsolute(generator)) {
            generator = path.join(authDir, generator);
        }

        if (fs.existsSync(generator)) {
            throw new Error('GameLoader.loadAuthDir: channel ' + gameName +
                ' custom generator not found: ' + generator);
        }
    }
    else {
        generator =
            path.join(this.servernode.rootDir, 'utils', 'codesGenerator.js');
 ...
 *      - log: if truthy, the message will be logged ('err' or 'warn')
 *      - err: an Error object to print the stack of
 *      - throwErr: if TRUE, the err value will be thrown instead
 *      - throwIt: if FALSE, the error will NOT be thrown (if 'warn' is FALSE)
...
GameLoader.prototype.doErr = function(msg, opts) {
    ...
    if (opts.throwIt !== false && opts.log !== 'warn') {
...
function parseGameSettings(settingsObj) {
    ...
    if (!settingsObj.treatments) {
        ...
        if (!out.standard.description) {
            out.standard.description = 'The standard treatment';
        }
    }
    else {
