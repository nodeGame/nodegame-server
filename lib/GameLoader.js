...
GameLoader.prototype.addGame = function(gameInfo, gameDir) {
    ...
    this.loadServerDir(gameDir, gameName);
    ...
    this.loadRequirementsDir(gameDir, gameName);
    ...
    logger.info('GameLoader.addGame: added ' + gameName + ': ' + gameDir);
};
...
/**
 * ### GameLoader.loadServerDir
 *
 * Loads several files from `server/` dir
 *
 ...
 * @see ServerChannel.createWaitingRoom
 */
GameLoader.prototype.loadServerDir = function(directory, gameName) {
    var settings, pwdFile, jwtFile;
    var channel;

    // 1. Channel settings.
    settings = this.loadChannelsFile(directory, gameName);

    // Save ref. to newly created channel.
    channel = this.servernode.channels[gameName];

    // 2. Credentials.
    pwdFile = directory + 'server/channel.credentials.js';

    loadSyncAsync(pwdFile, settings, 'loadServerDir', function(credentials) {
        // TODO: checkings. Add method.
        channel.credentials = credentials;
    });

    // 3. JWT secret.
    jwtFile = directory + 'server/channel.secret.js';

    loadSyncAsync(jwtFile, settings, 'loadServerDir', function(secret) {
        // TODO: checkings. Add method.
 ...
 * level of the specified directory.
 *
 * The file _channels.settings.js_ must export one channel object in the
...
GameLoader.prototype.loadChannelsFile = function(directory, gameName) {
    ...
    channelsFile = directory + '/server/channel.settings.js';
 ...
 * @param {string} gameName The name of the game
 */
GameLoader.prototype.loadRequirementsDir = function(directory, gameName) {
    ...
    this.servernode.channels[gameName]
        .createRequirementsRoom(J.merge(reqObj, settings));
};
        ...
        if (len === 1) {
            for (channel in gameChannelNames) {
                if (gameChannelNames.hasOwnProperty(channel)) {
                    servernode.channels[channel].adminServer[method](callback);
                    servernode.channels[channel].playerServer[method](callback);
                }
            }
        }
