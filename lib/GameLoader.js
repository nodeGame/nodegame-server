...
GameLoader.prototype.loadChannelFile = function(directory, gameName) {
    ...
    if (channelConf.name) {
        if ('string' !== typeof channelConf.name) {
            throw new TypeError('GameLoader.loadChannelFile: game: ' +
                                gameName + ' name must be string or ' +
                                'undefined. Found: ' + channelConf.name);
        }
    }
    else {
    ...
    if (channel) {
        ...
        if (channelConf.alias) {
            this.servernode.createGameAlias(channelConf.alias, gameName);
        }
...
GameLoader.prototype.loadAuthDir = function(directory, channel) {
    ...
    if (fs.existsSync(settings.codes)) {
        ...
        if (codes) importAuthCodes(channel, codes, settings);
    }
...
GameLoader.prototype.doErr = function(msg, opts) {
    ...
    if (opts.err) msg += '. Err:\n' + opts.err.stack;
    if (opts.log) {
        if ('boolean' === typeof opts.log || opts.log === 'err') {
