...
GameLoader.prototype.loadLevelsDir = function(directory, channel) {
    ...
    this.updateGameInfo(gameName, { levels: levels });
 ...
 */
GameLoader.prototype.loadGameDir = function(directory) {
    ...
    if ('string' !== typeof directory) {
        throw new TypeError('GameLoader.loadGameDir: directory must be ' +
                            'string.');
    }
    ...
    try {
        ...
        throw new Error('GameLoader.loadGameDir: cannot read ' +
                        gameSettingsPath + ': ' + e.stack);
    }

    if ('object' !== typeof gameSettings) {
        throw new Error('SeverNode.loadGameDir: invalid settings file: ' +
                        gameSettingsPath + '.');
    }

    if (gameSettings.TIMER) {
        if ('object' !== typeof gameSettings.TIMER) {
            throw new Error('SeverNode.loadGameDir: gameSettings.TIMER ' +
                            'must be object or undefined. File: ' +
                            gameSettingsPath + '.');
        }
    }
    else {
    ...
    if ('function' !== typeof gameStages) {
        throw new Error('SeverNode.loadGameDir: stages file did not ' +
    ...
    if ('function' !== typeof gameSetup) {
        throw new Error('SeverNode.loadGameDir: setup file did not ' +
    ...
    if ('object' !== typeof gameSetup) {
        throw new Error('SeverNode.loadGameDir: setup function did  ' +
    ...
    if ('object' !== typeof gameSettings) {
        throw new Error('SeverNode.loadGameDir: error parsing ' +
