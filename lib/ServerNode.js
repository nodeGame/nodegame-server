...
ServerNode.prototype.init = function(options) {
    ...
    // This is not working, because one file is still saved under the old
    // location and this creates a crash at the 10th restart of the server.
    // Also this does not redirect the stream for channel and messages.
    //    if (options.logDir) {
    //        if (this.logger.transports && this.logger.transports.file) {
    //            this.logger.transports.file.dirname =
...
ServerNode.prototype.addGame = function(gameInfo, treatments, gameDir) {
    ...
    if (!treatment.gamePaths.logic) {
        throw new Error('ServerNode.addGame: logic path must exist.');
    }
