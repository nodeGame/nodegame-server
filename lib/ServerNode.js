...
//     });
   ...
   //  });
    ...
    process.on('SIGTERM', function() {
        var chanIdx, pjsIdx
        var chan, pjs;
        for (chanIdx in that.channels) {
            if (that.channels.hasOwnProperty(chanIdx)) {
                chan = that.channels[chanIdx];
...
ServerNode.prototype.addChannel = function(options) {
    ...
    // Add the channel to the Page manager.
    // TODO: rename addGame to addChannel.
    this.pager.addGame(cname);
 ...
 *
 * Scans the directories listed in this.gamesDirs, looking for
 * package.json files.
 *
...
ServerNode.prototype.addGameDir = function(gameDir) {
    var gameInfo, gameSettings, gamePath, gameSettingsPath, gameSettingsObj;
 ...
 */
ServerNode.prototype.addGame = function(gameInfo, gameDir) {
    ...
    if (gameInfo.engines) {
        ...
        if (reqFail) {
            throw new Error('ServerNode.addGame: game ' + gameName + ' ' +
    ...
    // Loading treatments, settings, and stages.
    ...
    this.info.games[gameInfo.name] = {
        ...
        treatments: gameGame.treatments,
    ...
    // Done.
    this.logger.info('ServerNode.addGame: ' + gameName + ': ' + gameDir);
};
 ...
 */
ServerNode.prototype.createGameAlias = function(alias, gameName) {
    var i, len;
    ...
    for ( ; ++i < len ; ) {
        if ('string' !== typeof alias[i]) {
            throw new TypeError(
                'ServerNode.createGameAlias: alias must be string.');
        }
        if (this.info.games[alias[i]]) {
            throw new Error('ServerNode.createGameAlias: ' +
                            'alias must be unique: ' + alias[i] +
                            ' (' + gameName + ').');
        }

        this.info.games[alias[i]] = this.info.games[gameName];
        this.pager.createAlias(alias[i], gameName);
    }
};


/**
 * ### ServerNode.loadGameGameDir
 *
 ...
 *   'treatments', 'stager', and 'setup'.
 *
 * @see ServerNode.addGame
 * @see ServerNode.parseGameTreatments
 */
ServerNode.prototype.loadGameGameDir = function(directory) {
    var gameSetupPath, gameSetup;
    var gameTreatmentsPath, gameTreatments;
    var gameStagesPath, gameStages;
    var stager;

    if ('string' !== typeof directory) {
        throw new TypeError('ServerNode.loadGameGameDir: directory must be ' +
                            'string.');
    }

    // Treatments.
    gameTreatmentsPath = directory + 'game/game.treatments.js';

    try {
        gameTreatments = require(gameTreatmentsPath);
    }
    catch(e) {
        throw new Error('ServerNode.loadGameGameDir: cannot read ' +
                        gameTreatmentsPath + '.');
    }

    if ('object' !== typeof gameTreatments) {
        throw new Error('SeverNode.loadGameGameDir: invalid treatments file: ' +
                        gameTreatmentsPath + '.');
    }
    ...
    gameStages = gameStages(stager, gameTreatments);
    ...
    gameSetup = gameSetup(gameTreatments, gameStages);
    ...
    // Parsing the treatments object before returning.
    gameTreatments = this.parseGameTreatments(gameTreatments);

    if ('object' !== typeof gameTreatments) {
        throw new Error('SeverNode.loadGameGameDir: error parsing ' +
                        'treatment object: ' + gameTreatmentsPath + '.');
    }

    return {
        setup: gameSetup,
        treatments: gameTreatments,
        stager: stager
    };
};

/**
 * ### ServerNode.loadClientTypes
 *
 ...
 * @see ServerNode.addGame
 */
ServerNode.prototype.loadClientTypes = function(directory) {
    var clientTypesDir, fileNames;
    var clientType, clientTypes, clientTypePath;
    var i, len;

    if ('string' !== typeof directory) {
        throw new TypeError('ServerNode.loadClientTypes: directory must be ' +
                            'string.');
    }
    ...
    if (!J.existsSync(clientTypesDir)) {
        this.logger.error('ServerNode.loadClientTypes: game directory not ' +
                          'found: ' + dir);
        return;
    }
    ...
    for ( ; ++i < len ; ) {
        if (path.extname(fileNames[i]) === '.js') {
            clientTypePath = clientTypesDir + fileNames[i];
            if (!fs.statSync(clientTypePath).isDirectory()) {
                try {
                    clientType = require(clientTypePath);
                    clientTypes[fileNames[i].slice(0,-3)] = clientType;
                }
                catch(e) {
                    this.logger.error('ServerNode.loadClientTypes: cannot ' +
                                      'read ' + clientTypePath + '.');
                    throw e;
                }
            }
    ...
    if (!clientTypes.logic) {
        throw new Error('ServerNode.loadClientTypes: logic type not found.');
    }
    if (!clientTypes.player) {
        throw new Error('ServerNode.loadClientTypes: player type not found.');
    }
 ...
 */
ServerNode.prototype.loadWaitRoomFile = function(directory, gameName) {
    var waitRoomFile, conf, waitRoom;

    if ('string' !== typeof directory) {
        throw new TypeError('ServerNode.loadWaitRoomFile: directory myst be ' +
                            'string.');
    }

    waitRoomFile = directory + 'waitroom/waitroom.settings.js';

    if (!fs.existsSync(waitRoomFile)) {
        this.logger.error('ServerNode.loadWaitRoomFile: waitroom.settings.js ' +
                          'not found. Game: ' + gameName + '.');
        return;
    }

    try {
        conf = require(waitRoomFile);
    }
    catch(e) {
        this.logger.error('ServerNode.loadWaitRoomFile: error reading ' +
                          'waitroom.settings file. Game: ' + gameName + '.');
        throw e;
    }

    if ('object' !== typeof conf) {
        throw new TypeError('ServerNode.loadWaitRoomFile: room.settings file ' +
                            'must return a configuration object.');
    }

    if (conf.logicPath) {
        if ('string' === typeof conf.logicPath) {
            throw new TypeError('ServerNode.loadWaitRoomFile: configuration ' +
    ...
    waitRoom = this.channels[gameName].createWaitingRoom(conf);

    if (!waitRoom) {
        throw new Error('ServerNode.loadChannelsFile: could not create ' +
...
/**
 * ### ServerNode.parseGameTreatments
 *
 * Parses a game treatments object and builds the treatments object
 *
 * The treatment object always includes a 'standard' treatment plus
 * any other treatment specified in the 'treatments' object.
 *
 ...
 *
 * @return {object} out The parsed treatments object
 *
 * @see ServerNode.addGame
 */
ServerNode.prototype.parseGameTreatments = function(treatmentsObj) {
    var standard, t, tmp, out;
    var treatments;
    if ('object' !== typeof treatmentsObj) {
        throw new TypeError('ServerNode.parseGameSettings: treatmentsObj ' +
                            'must be object.');
    }

    out = {};

    if ('undefined' === typeof treatmentsObj.treatments) {
        out.standard = J.clone(treatmentsObj);
    }
    else {
        treatments = treatmentsObj.treatments;
        out.standard = J.clone(treatmentsObj);
        delete out.standard.treatments;
        for (t in treatments) {
            if (treatments.hasOwnProperty(t)) {
                tmp = J.clone(out.standard);
                J.mixin(tmp, treatments[t]);
                out[t] = tmp;
                out[t].name = t;
            }
        }
 ...
 *
 * @see ServerNode.loadConfDir
 * @see ServerNode.getChannelSandbox
 */
ServerNode.prototype.loadAuthDir = function(directory, gameName) {
    ...
    fs.exists(authFile, function(exists) {
        ...
        require(authFile)(authObj, that.info.games[gameName].treatments);
        ...
        if (len === 1) {
            for (channel in gameChannelNames) {
                if (gameChannelNames.hasOwnProperty(channel)) {
                    that.channels[channel].admin[method](callback);
                    that.channels[channel].player[method](callback);
                }
            }
        }
