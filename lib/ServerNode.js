...
function ServerNode(opts) {
     ...
     */
    this.modulesVersion =
        require(path.resolve(this.rootDir, 'bin', 'info.js')).modulesVersion;
...
ServerNode.prototype.addChannel = function(options) {
    var channelName, channelOptions, channel, ok, defaultChannel;
    ...
    if (options.defaultChannel) {
        if (this.getDefaultChannel()) {
            this.info.games[options.name].errored = true;
            throw new Error('ServerNode.addChannel: cannot set channel ' +
                            options.name + ' as default. Default channel ' +
                            'already set: ' + this.getDefaultChannel());
        }
        if (this.homePage.enabled) {
            this.info.games[options.name].errored = true;
            throw new Error('ServerNode.addChannel: cannot set channel ' +
                            options.name + ' as default. ServerNode has ' +
                            'home page enabled.');
        }
        defaultChannel = true;
    ...
    // Store references.
    this.info.channels[channelName] = J.merge(channelOptions, {open: true});
    this.channels[channelName] = channel;

    // Set default channel.
    if (defaultChannel) this.setDefaultChannel(channelName);
...
ServerNode.prototype.setDefaultChannel = function(channelName) {
    ...
    if (this.defaultChannel) {
        throw new Error('ServerNode.setDefaultChannel: default channel ' +
                        'cannot be changed after it has been set. Current ' +
...
function checkServerOptions(type, options, channels) {
    ...
    if ('undefined' === typeof options[type]) {
        ...
        throw new TypeError('ServerNode.addChannel: options.' + type +
                            'must be string, object, or undefined.');
    }
