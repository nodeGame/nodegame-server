...
ServerNode.prototype.addGameDir = function(gameDir) {
    ...
    gameSettings.gamePaths = clientTypes;
...
ServerNode.prototype.loadClientTypes = function(directory) {
    ...
    clientTypes = fs.readdirSync(clientTypesDir);

    gamePaths = {};

    i = -1, len = clientTypes.length;
    for ( ; ++i < len ; ) {
        if (path.extname(clientTypes[i]) === '.js') {
            clientTypePath = clientTypesDir + clientTypes[i];
            if (!fs.statSync(clientTypePath).isDirectory()) {
                // TODO: require.
                gamePaths[clientTypes[i].slice(0,-3)] = clientTypePath;
            }
        }
    }
...
// Was in addGame. It sets the correct paths to client types. Not needed
// at the moment.
...
//                     }
//                     catch(e) {
//                         this.logger.error('ServerNode.addGame: ' +
    // cannot read ' +
//                                           gamePath + '.');
//                         throw e;
//                     }
