...
function ServerNode(opts) {
    ...
    /**
     * ### ServerNode.nodeGameRoot
     *
     * The directory of launcher.js
     */
    ...
    /**
     * ### ServerNode.disabledChannels
     *
     * Object containing references to all disabled channels' settings
     */
...
ServerNode.prototype.init = function(options) {
    ...
    this.logger.verbose('ServerNode.init: starting HTTP server on port ' +
                        this.port);
 ...
 *   FALSE if channel is disabled or an error occurs
 *
 * @see ServerChannel
 */
ServerNode.prototype.addChannel = function(options) {
    var channelName, channelOptions, channel, ok, setDef;
    ...
    // If channel is disabled, store a reference and return false.
    if (options.enabled === false) {
        this.disabledChannels[channelName] = options;
        this.logger.warn('ServerNode.addChannel: disabled channel ' +
                         channelName);
        return false;
    }
    ...
    // TODO: or channelOptions?
    this.info.games[channelName].channel = options;
    // Adding channel aliases.
    if (options.alias) this.createGameAlias(options.alias, channelName);
...
ServerNode.prototype.addGameDir = function(gameDir, force) {
    var gameInfo, gamePath, channelName, defChannel;
    ...
    defChannel = this.getDefaultChannel();
    if (defChannel && !force) {
        channelName  = gameInfo.channelName || gameInfo.name;
        if (channelName !== defChannel) {
...
ServerNode.prototype.createGameAlias = function(alias, gameName, force) {
    ...
    if ('string' === typeof alias) alias = [alias];
...
ServerNode.prototype.ready = function(cb) {
    ...
    this.asyncQueue.onReady(function() {
        if (cb) cb();
        if (Object.isEmpty(that.channels)) {
            that.logger.warn('ServerNode: no channel added.');
        }
    });
};
...
ServerNode.prototype.getDefaultChannel = function() {
    // Fallsback to user option if defaultChannel isn't loaded yet.
    return this.defaultChannel || this._defaultChannel;
};
...
function checkServerOptions(type, options, channels) {
    ...
    if ('undefined' === typeof options[type]) {
        ...
        if ('undefined' === typeof options[type].endpoint) {
            options[type].endpoint = endpoint;
        }
 ...
 * ### resolvePath
 *
 * Returns a path as is if absolute, otherwise it resolves from cwd directory.
 *
