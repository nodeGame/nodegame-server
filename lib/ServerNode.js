...
function ServerNode (options) {
    ...
    this.confDir = options.confDir || this.rootDir + '/conf';
    ...
    // Loading loggers config
    this.loadConfFile(this.confDir + '/loggers.js', this.configureLoggers);
    
    this.logger = this.loggers.get('servernode');
    
edede
    
    this.logger.info('Loading configuration files...');
    this.loadConfDir(this.confDir);
...
ServerNode.prototype.addChannel = function (options) {
    ...
    var channel, ok; 
    try {
    	channel = new ServerChannel(this.server, options);
    	ok = channel.listen();
    }
    catch(e) {
    	this.logger.error(e);
    ...
    if (!ok) {
        this.logger.error('Channel could not be added: ' + options.name);
        return false;
    }
    ...
    this.logger.info('Channel added correctly: ' + options.name);
...
//		try {
//	    	conf = require(file);
//	        result = this.configureLoggers(conf);
//	        if (result) {
//	        	this.logger.info('Loggers file loaded correctly: ' + file);
//	        }
//	        else {
//	        	winston.warn('A non-fatal error occurred while loading the Loggers configuration file: ' + file);
//	        }	      
//	    }
//	    catch(e) {
//	    	winston.error('A fatal error was raised while loading the configuration files for the loggers: ' + file);
//	    	winston.error(e);
//	    	throw new Error(e);
//	    }
//	}
...
//
// if (!options) {
//     this.logger.error('Options are not correctly defined for the waiting room. Aborting');
//     return false;
// }
//    
// var wroom;
...
//// if (ok) {
////     this.channels.push(channel);
////     this.logger.error('Channel added correctly: ' + options.name);
//// }
//// else {
////     this.logger.error('Channel could not be added: ' + options.name);
//// }
