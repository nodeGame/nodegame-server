...
ServerNode.prototype.addGame = function(gameInfo, treatments, gameDir) {
    ...
    var gameLogicPath, gameLogic, gameClientPath, gameClient, gameBotPath, gameBot;
    ...
    for (t in treatments) {
        if  (treatments.hasOwnProperty(t)) {
            ...
            // Bot.
            if (treatment.botPath) {
                if ('string' !== typeof treatment.botPath) {
                    throw new TypeError('ServerNode.addGame: ' +
                                        'game ' + gameInfo.name + ' ' +
                                        'treatment ' + t + ': botPath ' +
                                        'must be string.');
                ...
                gameBotPath = gameDir + 'server/' + treatment.botPath;
                try {
                    gameBot = require(gameBotPath);
                }
                catch(e) {
                    this.logger.error('ServerNode.addGame: cannot read ' +
                                      gameBotPath + '.');
