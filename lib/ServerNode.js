...
ServerNode.prototype.init = function(options) {
    var that, gamesDirIndex, gamesInThisDirectiory, games, gamesIndex;
    ...
    // Parse directories to infer language structures.
    for (gamesDirIndex in this.gamesDirs) {
        gamesInThisDirectiory = fs.readdirSync(this.gamesDirs[gamesDirIndex]);

        // Extracting all the games from the various directories.
        games = gamesInThisDirectiory.filter(function(game){
            return !game.match(/^README.*/);
        });

        for (gamesIndex in games) {
            this.languages[games[gamesIndex]] =
                buildLanguagesArray(games[gamesIndex],
                    this.gamesDirs[gamesDirIndex]);
        }
    }
    ...
    // Builds array of language objects for one given game.
    function buildLanguagesArray(game,directory) {
        var ctxPath = directory + '/' + game + '/html/context/',
            langPaths = fs.readdirSync(ctxPath),
            languages, pathIndex, languageObject;

            languages = [];
            for (pathIndex in langPaths) {
                if (fs.existsSync(ctxPath + langPaths[pathIndex] +
                    '/languageInfo.json')) {
                        languageObject = JSON.parse(
                            fs.readFileSync(ctxPath + langPaths[pathIndex] +
                                '/languageInfo.json'));
                }
                else {
                    languageObject = {};
