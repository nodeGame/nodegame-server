...
ServerNode.prototype.addGame = function(game, gameDir) {
	...
	this.info.games[game.metadata.name] = {
	...
	if (game.alias) {
		...
		for (var i=0; i < game.alias.length; i++) {
			this.info.games[game.alias[i]] = this.info.games[game.metadata.name];
		}
	}
	
	this.loadConfDir(gameDir + '/server/conf');
	
	this.logger.info('New game added. ' + game.metadata.name + ': ' + gamePath);
...
ServerNode.prototype.loadConfFile = function (file, cb) {
	...
	if (!J.existsSync(file)) {
		this.logger.silly('An error has occurred. Could not open conf file: ' + file);
		return false;
	}
 ...
 * 
 * Refer to documentation for the content of each configuration file.
 * 
 ...
 * 	files are allowed to be loaded. Defaults, all configuration files are loaded.
 * @param {force} Optional. Force to rescan a directory even if this was already
 * 	loaded. Defaults, FALSE.
 * 
 * @see ServerNode.configure
 * @see ServerNode.configureHTTP
 * @see ServerNode.confiureSIO
...
ServerNode.prototype.loadConfDir = function (dir, mask, force) {
	...
	if (!force && this.scannedGamesDirs[dir]) {
		this.logger.warn('Duplicated conf directory found. Use the \'force\' flag to scan it again: ' + dir);
		return;
	}
