...
ServerNode.prototype.addChannel = function (options) {
    ...
    var channel, ok; 
//    try {
    	channel = new ServerChannel(this.sio, options);
    	ok = channel.listen();
//    }
//    catch(e) {
//    	this.logger.error(e);
...
ServerNode.prototype.startGame = function(channel, game, name) {
	if (!channel) {
	...
	this.logics[name] = node;
	
	node.socket.setSocketType('SocketDirect', {
		socket: this.channels[channel].admin.gmm.socket_manager.sockets.direct
	});
...
//// 
//// // TODO merge global options with local options
//// var channel = new ServerChannel(options, this.sio, this.io);
