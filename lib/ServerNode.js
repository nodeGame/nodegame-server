...
var exp = express();
var http = nodeHTTP.createServer(exp);
...
ServerNode.prototype.addChannel = function(options) {
    ...
    if ('string' === typeof options.playerServer) {
        options.playerServer = { endpoint: options.playerServer };
    }

    if ('object' !== typeof options.playerServer) {
        throw new TypeError('ServerNode.addChannel: ' +
                            'options.playerServer must be string or object.');
    }

    if ('string' === typeof options.adminServer) {
        options.adminServer = { endpoint: options.adminServer };
    }

    if ('object' !== typeof options.adminServer) {
        throw new TypeError('ServerNode.addChannel: ' +
    ...
    if (!channelOptions.name) channelOptions.name = channelOptions.gameName;
    ...
    this.resourceManager.addGame(cname);
...
ServerNode.prototype.loadConfFile = function(file, cb) {
    ...
    this.logger.info('ServerNode.loadConfFile: loaded ' + file);
};
