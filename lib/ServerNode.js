 ...
 *
 * @param {object} game A game descriptor
 * @param {string} gameDir The directory containing the game
 *
 * @see ServerNode.loadConfDir
 */
ServerNode.prototype.addGame = function(game, gameSettings, gameDir) {
    var gamePath, i, reqFail, req;
    var gameLogicPath, gameLogic, gameClientPath, gameClient;

    if ('object' !== typeof game) {
        throw new TypeError('ServerNode.addGame: game must be object.');
    }
    if ('string' !== typeof gameDir) {
    ...
    // Check name.
    if ('string' !== typeof game.name) {
        throw new Error('ServerNode.addGame: missing or invalid game name: ' +
                        gameDir + '.');
    }

    // Checking nodeGame server version requirement.
    if (game.engines && game.engines.nodegame) {
        
        req = game.engines.nodegame;
        
        // Little trick: we compare version numbers using the GameStage class.
        if (req.indexOf(">=") !== -1) {
            req = req.split(">=")[1];
        ...
        if (reqFail) {
            throw new Error('ServerNode.addGame: game ' + game.name + ' ' +
                            'requires nodeGame version ' + req  + ' ' +
                            'but found ' + this.version + '.');
        }
    }

    // Require Logic and Client files.

    gameLogicPath = gameDir + '/server/game.logic.js';
    try {
        gameLogic = require(gameLogicPath);
    }
    catch(e) {
        this.logger.error('ServerNode.addGame: an error occurred while ' +
                          'trying to read ' + gameLogicPath + '.');
        throw e;
    }

    gameClientPath = gameDir + '/server/game.client.js';
    try {
        gameLogic = require(gameClientPath);
    }
    catch(e) {
        this.logger.error('ServerNode.addGame: an error occurred while ' +
                          'trying to read ' + gameClientPath + '.');
        throw e;
    }
    ...
    this.info.games[game.name] = {
        ...
        game: game,
        treatments: gameSettings,
        gameLogic: gameLogic,
        gameLogicPath: gameLogicPath,
        gameClient: gameClient,
        gameClientPath: gameClientPath
    };
    
    this.logger.info('ServerNode: game added. ' + game.name + ': ' + gamePath);

    // Adding game alias.
    if (game.alias) {
        if ('string' === typeof game.alias) {
...
ServerNode.prototype.parseGameSettings = function(gameSettingsObj) {
    ...
    if (!gameSettingsObj.path2logic) {
        gameSettingsObj.path2logic = 'game.logic.js';
    }
    if (!gameSettingsObj.path2client) {
        gameSettingsObj.path2logic = 'game.client.js';
    }
    // TODO: do we need game room, or codes files, etc. ?
 ...
 * ### ServerNode.getGames
 *
 * Returns an object containing game and treatment information
 *
