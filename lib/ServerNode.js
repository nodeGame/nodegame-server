...
ServerNode.prototype.init = function(options) {
    ...
    this.logger.info('ServerNode: loading default configuration files.');
    ...
    if (options.confDir) {
        this.logger.info('ServerNode: loading user configuration files.');
    ...
    this.logger.info('ServerNode: starting HTTP server.');
    ...
    // Start error manager.
    this.logger.info('ServerNode: starting error manager.');
    that = this;
    ...
    // Loading games.
    this.logger.info('ServerNode: loading games.');
    this.loadGames(this.gamesDirs);

    // Done.
    this.logger.info('ServerNode: up and running on port ' + this.port + '.');
};
...
ServerNode.prototype.addChannel = function(options) {
    ...
    this.logger.info('ServerNode.addChannel: channel added correctly: ' + channelOptions.name);
...
ServerNode.prototype.addGame = function(game, gameDir) {
    ...
    // Check name
    if ('undefined' === typeof game.name || !game.name) {
