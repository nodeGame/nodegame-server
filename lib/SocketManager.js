...
SocketManager.prototype.broadcast = function(gameMsg, from) {
    ...
    fromRoomName = this.server.channel.registry.getClientRoom(from);
    if (!fromRoomName) {
        throw new Error('SocketManager.broadcast: Sender (ID: ' + from + ') is not in any room');
    }
    else {
    ...
    for (client in toClients) {
        if (toClients.hasOwnProperty(client)) {
            // TODO: resolve aliases for this check:
