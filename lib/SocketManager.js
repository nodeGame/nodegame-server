...
SocketManager.prototype.send2ownChannel = function(gameMsg, from) {  
    return this.send2channel(gameMsg, this.server.channel, from)
};
...
SocketManager.prototype.send2channel = function(gameMsg, channel, from) {
    var res;
    if ('undefined' !== from) gameMsg.from = from;

    res = send2AllClientsInGameServer(channel.admin);
    res = res && send2AllClientsInGameServer(channel.player);
    return res;
};
...
SocketManager.prototype.send2channelPlayers = function(gameMsg, channel, from) {
    if ('undefined' !== from) gameMsg.from = from;
    return send2AllClientsInGameServer(channel.player);
};
...
SocketManager.prototype.send2ownRoom = function(gameMsg, from) {
    ...
    fromRoomName = registry.getClientRoom(resolvedFrom);
    if (!fromRoomName) {
        throw new Error('SocketManager.send2ownRoom: Sender (ID: ' + 
                        resolvedFrom + ') is not in any room.');
    }
...
SocketManager.prototype.send2group = function(gameMsg, group) {
    ...
    if (!J.isArray(group)) {
        throw new TypeError('SocketManager.send2group: group must be array ' +
                            'or instance of PlayerList.', 'error');
    }
...
function send2AllClientsInGameServer(server) {
    var i, res, socketManager;
    socketManager = server.socket;
    for (i in socketManager) {
        if (socketManager.hasOwnProperty(i)) {
            res = socketManager.sockets[i].send(gameMsg, 'ALL');
            if (!res) {
...
SocketManager.prototype.broadcast2all = function(gameMsg, from) {
    ...
    broadcast2ownChannel(gameMsg, from);
    ...
    for (channelName in channels) {
        if (channelName !== gameMsg._channelName &&
            ...
            res = res && channel.admin.socket.send2ownChannel(gameMsg, from);
        }
     }
...
SocketManager.prototype.broadcast2channel = function(gameMsg, channel, from) {
    ...
    for (room in channel.gameRooms) {
        if (channel.gameRooms.hasOwnProperty(room)) {
            res = res && this.broadcast2room(gameMsg, channel.gameRooms[room],
                                            from);
        }
    }
...
SocketManager.prototype.broadcast2room = function(gameMsg, room, resolvedFrom) {
    ...
    for (client in toClients) {
        if (toClients.hasOwnProperty(client)) {
            // TODO: change back to '!==' once 'from' is string again
            if (client != resolvedFrom) {
                gameMsg.to = client;
                smoothExec = smoothExec && this.send2client(gameMsg);
            }
        }
    }
