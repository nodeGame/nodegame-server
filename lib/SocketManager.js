...
SocketManager.prototype.send2all = function(gameMsg) {
    ...
    for (i in this.sockets) {
        if (this.sockets.hasOwnProperty(i)) {
            ...
            if (!res) {
                this.sysLogger.log('SocketManager.send2all: An error occurred in socket: ' + i, 'error');
                smoothExec = false;
            }
        }
    }
...
SocketManager.prototype.send2client = function(gameMsg, to) {
    ...
    if (!resolvedInfo.success) {
        ...
        this.sysLogger.log('SocketManager.send2client: Msg not sent. ' + resolvedInfo.errMsg, 'error');
        return false;
    }
    ...
    if (!res) {
        errMsg = 'SocketManager.send2client: An error occurred while sending a message to ' +
...
SocketManager.prototype.send2room = function(gameMsg, room) {
    var res;
    res = send2clients.call(this, gameMsg, room.clients.id.getAllKeys());
    if (!res) this.sysLogger.log('SocketManager.send2room: an error occurred for room ' + room.name, 'error');
    return res;
};
...
SocketManager.prototype.send2roomAdmins = function(gameMsg, room) {
    var res;
    res = send2clients.call(this, gameMsg, room.clients.admin.fetchValues('id').id);
    if (!res) this.sysLogger.log('SocketManager.send2roomAdmins: an error has occurred for room ' + room.name, 'error');
    return res;
};
...
SocketManager.prototype.send2roomPlayers = function(gameMsg, room) {
    var res;
    res = send2clients.call(this, gameMsg, room.clients.player.fetchValues('id').id);
    if (!res) this.sysLogger.log('SocketManager.send2roomPlayers: an error has occurred for room ' + room.name, 'error');
    return res;
};
...
SocketManager.prototype.send2group = function(gameMsg, group) {
    ...
    if (!res) this.sysLogger.log('SocketManager.send2group: an error has occurred.', 'error');
    return res;
};
...
SocketManager.prototype.broadcast2group = function(gameMsg, group, from) {
    ...
    if (!res) this.sysLogger.log('SocketManager.broadcast2group: an error has occurred.', 'error');
    return res;
};
