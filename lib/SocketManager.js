...
SocketManager.prototype.send2ownRoom = function(gameMsg, from) {
    ...
    return this.send2room(gameMsg, fromRoom, resolvedFrom);
};
...
SocketManager.prototype.broadcast = function(gameMsg, from) {
    ...
    case 'ALL':
        return this.broadcast2all(gameMsg);

    case 'CHANNEL':
        return this.broadcast2ownChannel(gameMsg);

    case 'ROOM':        
        return this.broadcast2ownRoom(gameMsg);
        
    default:
        toObj = this.getToFieldObject(gameMsg);
        if (toObj) {
            if (gameMsg.to.lastIndexOf('CHANNEL_', 0) === 0) {
                this.broadcast2channel(gameMsg, toObj);
            }
            else {
                this.broadcast2room(gameMsg, toObj);
            }
        }
 ...
 * @return {boolean} FALSE, if at least one error occurs
 */
SocketManager.prototype.broadcast2all = function(gameMsg) {
    var channels, channelName, res;
 ...
 * @return {boolean} FALSE is an error occurs
 */
SocketManager.prototype.broadcast2channel = function(gameMsg, channel) {
    var res, room;
    res = true;
    // Waiting room is included here.
    for (room in channel.gameRooms) {
