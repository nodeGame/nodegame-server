...
// Listens on newly connected players		
	this.on(say + 'HI', function(msg) {
		...
		log.log('Incoming monitor: ' + msg.from);
		
		// Notify other monitors that a new one connected
		that.pl.each(function(p) {
			that.gmm.sendMCONNECT(msg.data, p.id); // or sid?
		});
		...
		that.gmm.sendMLIST(that, msg.from);
...
// Forwards a redirect msg (only for admin)
	this.on(say + 'REDIRECT', function (msg) {
		// TODO refactor gmm
		var msg = that.gmm.gmg.sayREDIRECT(msg.data, msg.to);
...
//    	that.pl.remove(id);
		...
		that.gmm.sendMDISCONNECT(monitor);
    });
